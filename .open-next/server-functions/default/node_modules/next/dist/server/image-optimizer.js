...
_export(exports, {
    ...
    canDecodeAvif: function() {
        return canDecodeAvif;
    },
    ...
    isAvifDecodeSafe: function() {
        return isAvifDecodeSafe;
    },
...
function getSharp(concurrency) {
    ...
    try {
        ...
        if (isAvifDecodeSafe(((_sharp_versions = _sharp.versions) == null ? void 0 : _sharp_versions.heif) ?? null)) {
            _sharp.unblock({
                operation: [
                    'VipsForeignLoadHeif'
                ]
...
function isAvifDecodeSafe(heifVersion) {
    if (heifVersion === null) {
        return false;
    }
    const [major = Number.NaN, minor = Number.NaN, patch = Number.NaN] = heifVersion.split('.').map((part)=>Number.parseInt(part, 10));
    if (Number.isNaN(major) || Number.isNaN(minor) || Number.isNaN(patch)) {
        return false;
    }
    return major > 1 || major === 1 && minor > 23 || major === 1 && minor === 23 && patch >= 2;
}
let _avifDecodeSafe = null;
function canDecodeAvif(concurrency) {
    if (_avifDecodeSafe === null) {
        let heifVersion = null;
        try {
            var _getSharp_versions;
            // Custom libvips builds do not report dependency versions, so the
            // absence of a heif version is treated as unsafe.
            heifVersion = ((_getSharp_versions = getSharp(concurrency).versions) == null ? void 0 : _getSharp_versions.heif) ?? null;
        } catch  {
...
async function imageOptimizer(imageUpstream, paramsResult, nextConfig, opts) {
    ...
    if (BYPASS_TYPES.includes(upstreamType) || upstreamType === AVIF && !canDecodeAvif(nextConfig.experimental.imgOptConcurrency)) {
        return {
            buffer: upstreamBuffer,
            contentType: upstreamType,
            maxAge,

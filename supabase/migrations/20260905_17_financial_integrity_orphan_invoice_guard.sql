...
declare
  ...
  v_orphan_invoice_journals bigint;
...
begin
  ...
  where je.status='posted' and je.source_type='invoice'
    and not exists(select 1 from public.invoices i where i.id=je.source_id);
  ...
  return jsonb_build_object(
    ...
    'journals', jsonb_build_object('unbalanced_posted',v_unbalanced_journals,'orphan_posted_invoice_journals',v_orphan_invoice_journals),
    'settlements', jsonb_build_object('duplicate_cash_legs',v_settlement_cash_duplicates),
    'pool_balances',v_pool,
    'healthy', (v_customer_mismatch=0 and v_customer_unseeded=0 and v_unbalanced_journals=0 and v_orphan_invoice_journals=0 and v_settlement_cash_duplicates=0)
  );
end;
$$;

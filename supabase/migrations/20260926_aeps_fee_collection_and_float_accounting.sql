-- AEPS Fee Collection Modes, Float Accounting & QR Settlement Migration
-- 1. Expand transactions_fee_source_check to explicitly include 'qr' alongside 'upi' and 'cut_from_withdrawal'
...
-- 2. Update create_business_txn with authoritative handling for:
--    - Cut from Withdrawal: Net cash out = Amount - Fee; Float credit = Amount + Commission
--    - Collect Separately (Cash): Cash out = Amount; Cash in = Fee; Float credit = Amount + Commission
--    - Collect Separately in QR: Cash out = Amount; Physical till cash in = 0; QR in = Fee; Float credit = Amount + Commission
--    - Payment Collection (Aadhaar Pay): Cash out = 0; Float credit = Amount + Commission
--    - Balance Enquiry / Mini Statement: Cash out = 0; Float credit = Commission
...
begin
  ...
  if p_service_type = 'aeps' then
    ...
    if v_norm_method in ('payment_collection', 'aadhaar_pay', 'collection') then
      ...
      if p_fee_source in ('separate_cash', 'customer_paid_extra') and v_fee > 0 then
        v_cash_in := v_fee;
        v_upi_fee := 0;
      elsif p_fee_source in ('upi', 'qr') and v_fee > 0 then
        v_cash_in := 0;
        v_upi_fee := v_fee;
      else
        v_cash_in := 0;
      ...
      -- 3. CASH WITHDRAWAL (Shop / Agent -> Customer, Direction: OUT)
      ...
      if p_fee_source in ('upi', 'qr') then
        -- Collect Separately in QR: full payout handed in cash; fee collected via QR
        -- Physical cash till must strictly NOT receive QR cash
        v_cash_out := p_amount;
        v_cash_in := 0;
        v_upi_fee := v_fee;
      elsif p_fee_source in ('separate_cash', 'customer_paid_extra') then
        -- Collect Separately (Cash): full payout handed in cash; fee collected in cash
        v_cash_out := p_amount;
        v_cash_in := v_fee;
        v_upi_fee := 0;
      else
        -- Cut from Withdrawal (cut_from_withdrawal / default): net payout handed in cash
        v_cash_out := p_amount - v_fee;
        v_cash_in := 0;
        v_upi_fee := 0;
      end if;

      -- AEPS Float / Balance is credited with Withdrawal Amount + Portal Commission
  ...
  if p_status = 'success' then
    ...
    -- 2b. QR Fee Inflow Leg (When fee collected separately via QR)
    if v_upi_fee > 0 then
      if p_merchant_qr_id is not null then
        select payment_instrument_id into v_qr_instrument_id
        from public.upi_merchant_qrs
      ...
      if v_qr_instrument_id is null then
        v_qr_instrument_id := v_cash_instrument_id;
      end if;

      insert into public.cash_entries (entry_date, method, direction, amount, description, ref_type, ref_id, instrument_id)
      values (p_transaction_date, 'upi', 'in', v_upi_fee, v_label || ' ' || v_number || ' fee received via QR', 'transaction', v_txn_id, v_qr_instrument_id);
    end if;
  ...
  values (
    ...
    jsonb_build_object('service_type', p_service_type, 'amount', p_amount, 'status', p_status, 'reference', p_reference, 'transfer_method', p_transfer_method, 'direction', v_direction, 'fee_source', p_fee_source)
  );
  ...
  return (
    select jsonb_build_object('id', id, 'transaction_number', transaction_number,
      ...
      'upi_fee', upi_fee, 'transfer_method', transfer_method, 'fee_source', fee_source)

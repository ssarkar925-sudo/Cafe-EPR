...
-- append-only cash-entry correction semantics.

BEGIN;

-- ============================================================
-- 1. Cash entries must never be rewritten by a dedupe trigger
-- ============================================================
...
BEGIN
  ...
  -- Resolve the instrument only when the caller omitted it. Never update or
  -- delete an existing entry: financial history is append-only.
...
-- ============================================================
-- 2. Multi-tender invoice collection: explicit back-office gate
-- ============================================================
...
BEGIN
  ...
  IF v_invoice.status IN ('cancelled','returned') THEN
    RAISE EXCEPTION 'Cannot pay a cancelled or returned invoice';
  END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_allocations) LOOP
    ...
    ELSE
      ...
      ELSIF v_method IN ('bank','wallet','cash') THEN
        SELECT id INTO v_inst FROM public.payment_instruments WHERE is_active AND lower(type)=v_method ORDER BY created_at ASC LIMIT 1;
      END IF;
    ...
    VALUES(p_invoice_id,v_method,v_amount,v_inst);
  ...
  SET paid=coalesce(paid,0)+v_total,
      due=v_due_after,
      status=CASE WHEN v_due_after<=0.005 THEN 'paid' ELSE 'partial' END
  ...
  IF v_customer_id IS NOT NULL THEN
    ...
    VALUES(v_customer_id,current_date,'payment','Payment on '||v_invoice.invoice_number||' ('||jsonb_array_length(p_allocations)||' tenders)',v_total,(SELECT balance FROM public.customers WHERE id=v_customer_id),p_invoice_id);
  END IF;
...
-- ============================================================
-- 3. Transaction customer payment split: explicit back-office gate
-- ============================================================
...
AS $function$
DECLARE
  v_total numeric:=round(coalesce(p_amount,0)+coalesce(p_service_fee,0)+coalesce(p_portal_charge,0),2);
  v_paid numeric:=0; v_due numeric; v_item jsonb; v_method text; v_inst uuid; v_result jsonb; v_txn uuid;
BEGIN
  IF auth.role()<>'service_role' THEN
    IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
    IF NOT public.is_back_office() THEN RAISE EXCEPTION 'Forbidden'; END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_customer_collection_allocations) LOOP
    v_paid:=v_paid+round(coalesce((v_item->>'amount')::numeric,0),2);
  END LOOP;
...
-- Keep wrapper ACLs: authenticated + service_role only, never anon/public.

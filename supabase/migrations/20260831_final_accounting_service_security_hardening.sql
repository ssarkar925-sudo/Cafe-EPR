...
-- The deployed migration defines post_journal_entry(), immutable posted entries, deferred balance validation,
-- and triggers for cash_entries, stock_movements and settlements. It also enables RLS on accounting tables,
-- adds missing FK indexes, consolidates duplicate policies, removes anonymous SECURITY DEFINER execution,
-- and makes trial_balance security-invoker.

alter view public.trial_balance set (security_invoker=true);

-- Never expose SECURITY DEFINER functions to anonymous callers through the public role.
...
-- Audit data is never directly writable/readable by anonymous clients.
...
-- Search-path hardening for new helper functions.
alter function public.accounting_asset_code(text) set search_path=public;
alter function public.trg_journal_immutable() set search_path=public;
alter function public.trg_journal_entry_immutable() set search_path=public;
alter function public.trg_journal_balance_check() set search_path=public;

...
DECLARE
  v_name text:=btrim(NEW.display_name); v_upi text:=lower(btrim(NEW.upi_id)); v_instrument_id uuid; v_instrument_type text; v_match_count integer:=0;
BEGIN
  ...
  IF NEW.payment_instrument_id IS NOT NULL THEN
    ...
    RETURN NEW;
  END IF;
  SELECT count(*) INTO v_match_count FROM public.payment_instruments WHERE lower(type) IN ('upi','upi_qr') AND lower(btrim(COALESCE(details->>'upi_id','')))=v_upi;

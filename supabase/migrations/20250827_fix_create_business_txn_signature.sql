-- ==============================================================================
-- Migration: Fix create_business_txn RPC signature mismatch
-- ==============================================================================
-- Problem: Frontend calls create_business_txn with 33 params (including
-- p_pay_from_instrument_id, p_pay_from_method, p_receiver_name) but production
-- has older overloads (31-32 params) causing "function not unique" or "not found".
-- 
-- Solution: Drop ALL obsolete overloads, create exactly ONE canonical 33-param
-- function that:
--   1. Accepts p_pay_from_instrument_id (uuid, references payment_instruments)
--   2. Accepts p_pay_from_method (text, default 'bank')
--   3. Accepts p_receiver_name (text)
--   4. Stores all three in transactions table
--   5. If p_pay_from_instrument_id provided, fetches instrument name and uses it
--      as pay_from_method (with fallback to provided p_pay_from_method)
--   6. Preserves all AEPS, DMT, UPI, recharge logic
-- ==============================================================================

-- -----------------------------------------------------------------------------
-- STEP 1: Inspect current signatures (run this in Supabase SQL Editor FIRST)
-- -----------------------------------------------------------------------------
-- SELECT proname, pg_get_function_identity_arguments(oid) as signature
-- FROM pg_proc
-- WHERE proname = 'create_business_txn'
--   AND pronamespace = 'public'::regnamespace
-- ORDER BY oid;

-- -----------------------------------------------------------------------------
-- STEP 2: Drop ALL existing create_business_txn overloads
-- -----------------------------------------------------------------------------

-- Drop 31-param: base version (schema.sql, fix-aeps-float-calculation.sql)
...
-- Drop 32-param: with p_receiver_name (add-upi-receiver-name.sql)
DROP FUNCTION IF EXISTS public.create_business_txn(
  p_service_type text,
  p_transaction_date date,
  p_transaction_timestamp timestamptz,
...
-- Drop 33-param: with p_receiver_name + p_customer_pay_method (fix-aeps-float-calculation.sql revoke)
DROP FUNCTION IF EXISTS public.create_business_txn(
  p_service_type text,
  p_transaction_date date,
  p_transaction_timestamp timestamptz,
...
-- -----------------------------------------------------------------------------
-- STEP 3: Create CANONICAL create_business_txn (33 parameters)
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Resolve pay_from_method: use instrument type if provided, otherwise use supplied/default
  if p_pay_from_instrument_id is not null then
    select coalesce(type, 'bank') into v_final_pay_from_method from public.payment_instruments where id = p_pay_from_instrument_id;
  else
    v_final_pay_from_method := coalesce(p_pay_from_method, 'bank');
  end if;

  -- Service-type specific logic (preserves ALL existing behavior)
  if p_service_type = 'aeps' then
    ...
    v_direction := 'out'; v_prefix := 'UPI'; v_seq := 'public.upi_seq';
  ...
  -- Insert the transaction (all columns explicitly listed)
  insert into public.transactions (
    id, transaction_number, direction, service_type,
    transaction_date, transaction_timestamp,
    customer_id, customer_mobile, reference, remarks, status,
    bank_id, portal_id, merchant_qr_id,
    aadhaar_last4, transfer_method,
    sender_name, sender_mobile,
    beneficiary_name, beneficiary_mobile,
    beneficiary_bank, beneficiary_ifsc, beneficiary_account,
    upi_id,
    amount, service_fee, portal_commission,
    fee_source, paid_from, customer_pay_method,
    pay_from_instrument_id, pay_from_method, receiver_name,
    cash_in, cash_out, bank_in, bank_out,
    pool_credit, pool_out, pool_credit_type, upi_fee
  ) values (
    gen_random_uuid(), v_number, v_direction, p_service_type,
    p_transaction_date, p_transaction_timestamp,
    p_customer_id, p_customer_mobile, p_reference, p_remarks, p_status,
    p_bank_id, p_portal_id, p_merchant_qr_id,
    p_aadhaar_last4, p_transfer_method,
    p_sender_name, p_sender_mobile,
    p_beneficiary_name, p_beneficiary_mobile,
    p_beneficiary_bank, p_beneficiary_ifsc, p_beneficiary_account,
    p_upi_id,
    p_amount, v_fee, p_portal_commission,
    p_fee_source, p_paid_from, p_customer_pay_method,
    p_pay_from_instrument_id, v_final_pay_from_method, p_receiver_name,
    v_cash_in, v_cash_out, v_bank_in, v_bank_out,
    v_pool_credit, v_pool_out, v_pool_type,
    v_upi_fee
  );
  ...
  return jsonb_build_object(
    ...
    'id', (select id from public.transactions where transaction_number = v_number)
  );
end;
$function$;

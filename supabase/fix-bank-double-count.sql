-- ============================================================
-- CORRECT FIX: get_pool_movements - bank pool
-- Problem: DMT-0001 counted twice:
--   1) via cash_entries (bank OUT 1500) - new code writes this
--   2) via transactions.bank_out (1500) - original RPC writes this
-- Fix: exclude transactions.bank_out where cash_entries already exists
-- for that transaction (no_double_count subquery)
-- ============================================================
  ...
  if p_pool = 'cash' then
    ...
    select coalesce(sum(x), 0) into v from (
      ...
      select -amount from public.settlements where status = 'success' and from_pool = 'bank'
        and settlement_date >= p_from and (p_to is null or settlement_date <= p_to)
      union all
      -- cash_entries for bank (single source of truth for transactions with cash_entries)
      ...
      union all
      -- transactions.bank_in ONLY for transactions NOT already in cash_entries
      -- (prevents double count for new transactions written to cash_entries)
      select bank_in from public.transactions t where status = 'success' and bank_in > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
        and not exists (
          select 1 from public.cash_entries ce
          where ce.ref_type = 'transaction' and ce.ref_id = t.id
            and ce.method in ('bank', 'debit_card', 'card') and ce.direction = 'in'
        )
      union all
      -- transactions.bank_out ONLY for transactions NOT already in cash_entries
      -- (prevents double count for new transactions written to cash_entries)
      select -bank_out from public.transactions t where status = 'success' and bank_out > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
        and not exists (
          select 1 from public.cash_entries ce
          where ce.ref_type = 'transaction' and ce.ref_id = t.id
            and ce.method in ('bank', 'debit_card', 'card') and ce.direction = 'out'
        )
    ) t;

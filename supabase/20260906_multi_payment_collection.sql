...
BEGIN
  ...
  END LOOP;
  IF v_total > greatest(0,coalesce(v_invoice.total,0)-coalesce(v_invoice.paid,0)) + 0.005 THEN
    RAISE EXCEPTION 'Payment allocations exceed outstanding due';
  END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_allocations) LOOP
    ...
    IF v_inst IS NOT NULL THEN
      ...
      v_method := CASE v_inst_type WHEN 'cash' THEN 'cash' WHEN 'bank' THEN 'bank' WHEN 'wallet' THEN 'wallet'
        WHEN 'upi' THEN 'upi' WHEN 'upi_qr' THEN 'upi' WHEN 'debit_card' THEN 'debit_card' WHEN 'credit_card' THEN 'credit_card' ELSE NULL END;
    ...
    INSERT INTO public.cash_entries(entry_date,method,direction,amount,description,ref_type,ref_id,instrument_id)
      VALUES(current_date,v_method,'in',v_amount,'Payment '||v_invoice.invoice_number||' ('||upper(v_method)||')','invoice',p_invoice_id,v_inst);
  END LOOP;
  ...
  IF v_customer_id IS NOT NULL THEN
    ...
    INSERT INTO public.customer_ledger(customer_id,entry_date,type,description,credit,balance_after,ref_id)
      VALUES(v_customer_id,current_date,'payment','Payment on '||v_invoice.invoice_number||' ('||jsonb_array_length(p_allocations)||' tenders)',v_total,
        (SELECT balance FROM public.customers WHERE id=v_customer_id),p_invoice_id);
  END IF;
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_txn record; v_item jsonb; v_method text; v_inst uuid; v_inst_type text; v_amount numeric;
  v_total numeric := 0; v_due numeric; v_prev numeric; v_new numeric;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_customer_collection_allocations) LOOP
    ...
    IF v_inst IS NULL THEN
      IF v_method='card' THEN SELECT id INTO v_inst FROM public.payment_instruments WHERE is_active AND lower(type) IN ('debit_card','credit_card') ORDER BY created_at ASC LIMIT 1;
      ELSIF v_method='upi' THEN SELECT id INTO v_inst FROM public.payment_instruments WHERE is_active AND lower(type) IN ('upi','upi_qr') ORDER BY created_at ASC LIMIT 1;
      ELSE SELECT id INTO v_inst FROM public.payment_instruments WHERE is_active AND lower(type)=v_method ORDER BY created_at ASC LIMIT 1; END IF;
    END IF;
    IF v_inst IS NULL THEN RAISE EXCEPTION 'No active payment instrument configured for %',v_method; END IF;

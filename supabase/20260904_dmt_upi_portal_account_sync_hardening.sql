...
DECLARE
  ...
  v_existing_id uuid;
BEGIN
  IF COALESCE(NEW.service_type, 'aeps') <> 'aeps' THEN
    RETURN NEW;
  END IF;
  ...
  IF NEW.payment_instrument_id IS NOT NULL THEN
    ...
    WHERE payment_instrument_id = NEW.payment_instrument_id
      ...
      AND COALESCE(service_type, 'aeps') = 'aeps'
    LIMIT 1;
...
BEGIN
  IF lower(COALESCE(NEW.type,'')) NOT IN ('dmt','dmt_portal') THEN
    IF TG_OP = 'UPDATE' THEN
      ...
      WHERE payment_instrument_id = NEW.id
        AND service_type = 'dmt'
      LIMIT 1;
      IF v_linked_instrument_id IS NOT NULL THEN
        RAISE EXCEPTION 'Cannot change DMT payment account type while it is linked to a portal';
      END IF;
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF v_name = '' THEN
    RAISE EXCEPTION 'DMT payment account name is required';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended('portal:dmt:' || lower(v_name), 0));
  NEW.name := v_name;

  IF current_setting('app.dmt_portal_bootstrap', true) = 'on' THEN
    RETURN NEW;
  END IF;

  SELECT id INTO v_portal_id
  FROM public.aeps_portals
  WHERE service_type = 'dmt'
    AND payment_instrument_id = NEW.id
  LIMIT 1;
  ...
  LIMIT 1;
  IF v_portal_id IS NOT NULL THEN
    RAISE EXCEPTION 'DMT portal % is already linked to another payment account', v_name;
  END IF;

  INSERT INTO public.aeps_portals (name, service_type, is_active, payment_instrument_id)
  VALUES (v_name, 'dmt', NEW.is_active, NEW.id);
...
DECLARE
  ...
  v_type text;
BEGIN
  IF COALESCE(NEW.service_type, 'aeps') <> 'dmt' THEN
    RETURN NEW;
  END IF;

  IF v_name = '' THEN
    RAISE EXCEPTION 'DMT portal name is required';
  END IF;
  ...
  IF NEW.payment_instrument_id IS NOT NULL THEN
    SELECT id, lower(type) INTO v_instrument_id, v_type
    FROM public.payment_instruments
    WHERE id = NEW.payment_instrument_id;

    IF v_instrument_id IS NULL THEN
      RAISE EXCEPTION 'DMT payment account % not found', NEW.payment_instrument_id;
    END IF;
    IF v_type NOT IN ('dmt','dmt_portal') THEN
      RAISE EXCEPTION 'Payment account % is not a DMT account', NEW.payment_instrument_id;
    END IF;

    SELECT id INTO v_existing_id
    FROM public.aeps_portals
    WHERE service_type = 'dmt'
      AND payment_instrument_id = NEW.payment_instrument_id
      AND id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid)
    LIMIT 1;
    IF v_existing_id IS NOT NULL THEN
      RAISE EXCEPTION 'DMT payment account is already linked to another portal';
    END IF;
  ...
  SELECT id INTO v_instrument_id
  FROM public.payment_instruments
  WHERE lower(type) IN ('dmt','dmt_portal')
    AND lower(btrim(name)) = lower(v_name)
  ORDER BY created_at, id
  LIMIT 1;
  ...
  IF v_instrument_id IS NULL THEN
    ...
    BEGIN
      INSERT INTO public.payment_instruments
        (name, type, is_active, created_by, details, opening_balance, current_balance)
      VALUES
        (v_name, 'dmt_portal', COALESCE(NEW.is_active, true), auth.uid(), '{}'::jsonb, 0, 0)
      RETURNING id INTO v_instrument_id;
    EXCEPTION WHEN unique_violation THEN
      SELECT id INTO v_instrument_id
      FROM public.payment_instruments
...
CREATE UNIQUE INDEX IF NOT EXISTS aeps_portals_dmt_payment_instrument_uidx
  ON public.aeps_portals (payment_instrument_id)
  WHERE service_type = 'dmt' AND payment_instrument_id IS NOT NULL;
...
AS $function$
DECLARE
  v_service text := lower(COALESCE(NEW.service_type,''));
  v_portal_service text;
  v_portal_instrument_type text;
BEGIN
  IF v_service IN ('aeps','dmt') AND NEW.portal_id IS NOT NULL THEN
    SELECT lower(COALESCE(service_type,'aeps')), lower(pi.type)
      INTO v_portal_service, v_portal_instrument_type
    FROM public.aeps_portals ap
    LEFT JOIN public.payment_instruments pi ON pi.id = ap.payment_instrument_id
    WHERE ap.id = NEW.portal_id;

    IF v_portal_service IS NULL THEN
      RAISE EXCEPTION '% portal not found', upper(v_service);
    END IF;
    IF v_portal_service <> v_service THEN
      RAISE EXCEPTION '% transaction cannot reference a % portal', upper(v_service), upper(v_portal_service);
    END IF;
    IF v_service = 'aeps' AND v_portal_instrument_type IS NOT NULL AND v_portal_instrument_type <> 'aeps_portal' THEN
      RAISE EXCEPTION 'AEPS portal is not linked to an AEPS payment account';
    END IF;
    IF v_service = 'dmt' AND v_portal_instrument_type IS NOT NULL AND v_portal_instrument_type NOT IN ('dmt','dmt_portal') THEN
      RAISE EXCEPTION 'DMT portal is not linked to a DMT payment account';
    END IF;
  END IF;
  RETURN NEW;
END;
...
BEGIN
  IF NEW.settlement_type = 'aeps_to_bank' AND NEW.source_instrument_id IS NOT NULL THEN
    ...
    IF v_src_type NOT IN ('aeps','aeps_portal') THEN
      RAISE EXCEPTION 'AEPS settlement source must be an AEPS payment account';
    END IF;
  END IF;
  ...
  IF NEW.settlement_type IN ('bank_to_dmt','wallet_to_dmt') AND NEW.dest_instrument_id IS NOT NULL THEN
    ...
    IF v_dst_type NOT IN ('dmt','dmt_portal') THEN
      RAISE EXCEPTION 'DMT settlement destination must be a DMT payment account';
    END IF;
  END IF;

  IF NEW.settlement_type IN ('upi_qr_to_bank','upi_qr_to_wallet') AND NEW.source_instrument_id IS NOT NULL THEN
    SELECT lower(type) INTO v_src_type FROM public.payment_instruments WHERE id = NEW.source_instrument_id;
    IF v_src_type NOT IN ('upi','upi_qr') THEN
      RAISE EXCEPTION 'UPI settlement source must be a UPI payment account';
    END IF;
  END IF;
...
CREATE UNIQUE INDEX IF NOT EXISTS upi_merchant_qrs_payment_instrument_uidx
  ON public.upi_merchant_qrs (payment_instrument_id)

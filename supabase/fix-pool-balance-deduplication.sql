-- Run this in Supabase SQL Editor to fix Opening Balances & Pool Movements calculation.
-- Fixes double deduction of bank movements (was summing both cash_entries and transactions.bank_out).
...
begin
  ...
  if p_pool = 'cash' then
    ...
    select coalesce(sum(case when direction = 'in' then amount else -amount end), 0) into v
    from public.cash_entries
    where method in ('bank', 'debit_card', 'card') and entry_date >= p_from and (p_to is null or entry_date <= p_to);
    ...
    select coalesce(sum(case when direction = 'in' then amount else -amount end), 0) into v
    from public.cash_entries
    where method = 'wallet' and entry_date >= p_from and (p_to is null or entry_date <= p_to);
    ...
    select coalesce(sum(x), 0) into v from (
      ...
      select -amount from public.settlements where status = 'success' and from_pool = 'dmt'
        and settlement_date >= p_from and (p_to is null or settlement_date <= p_to)
      union all
      select -pool_out from public.transactions where status = 'success' and service_type = 'dmt' and paid_from = 'portal' and pool_out > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
    ) t;

  elsif p_pool = 'aeps' then
    select coalesce(sum(x), 0) into v from (
      select pool_out as x from public.transactions where status = 'success' and service_type = 'aeps' and pool_out > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
      union all
    ...
    select coalesce(sum(x), 0) into v from (
      select pool_credit as x from public.transactions where status = 'success' and service_type = 'upi' and pool_credit > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
      union all
    ...
    select coalesce(sum(x), 0) into v from (
      ...
      select -amount from public.settlements where status = 'success' and from_pool = 'recharge'
        and settlement_date >= p_from and (p_to is null or settlement_date <= p_to)
      union all
      select -pool_out from public.transactions where status = 'success' and service_type = 'recharge' and pool_out > 0
        and transaction_date >= p_from and (p_to is null or transaction_date <= p_to)
    ) t;
...
-- Deduplicate any duplicate cash_entries (keeping latest 1 per transaction & direction)

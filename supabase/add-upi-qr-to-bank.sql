-- Run this in Supabase SQL Editor (idempotent). Adds the UPI QR Wallet -> Bank settlement.
-- Model: Bank payments land in the Bank pool directly; UPI payments land in the merchant
-- QR wallet (upi_qr pool); this settlement moves that wallet balance into the Bank.

-- 1) Allow the new settlement type on the settlements table (drop whatever check exists first).
do $$
declare c text;
begin
...
-- 2) Redefine create_settlement with the new branch (keeps all existing types + recharge).

-- ==============================================================================
-- Add Bank to Wallet Settlement Type (BTW) to settlements & create_settlement
-- ==============================================================================
...
alter table public.settlements add constraint settlements_settlement_type_check
  check (settlement_type in (
    ...
    'upi_qr_to_bank', 'wallet_to_bank', 'bank_to_wallet', 'bank_to_recharge',
    'recharge_to_bank', 'recharge_to_wallet', 'bank_to_credit_card',
    'cash_to_credit_card', 'credit_card_to_bank', 'bank_withdrawal',
    'add_cash_to_bank', 'cash_adjustment'
  ));

alter table public.settlements add constraint settlements_from_pool_check
  check (from_pool in ('cash', 'bank', 'wallet', 'dmt', 'aeps', 'upi_qr', 'credit_card', 'recharge'));

alter table public.settlements add constraint settlements_to_pool_check
  check (to_pool in ('cash', 'bank', 'wallet', 'dmt', 'aeps', 'upi_qr', 'credit_card', 'recharge'));
...
grant execute on function public.create_settlement(text, date, numeric, text, text, text) to authenticated, service_role;

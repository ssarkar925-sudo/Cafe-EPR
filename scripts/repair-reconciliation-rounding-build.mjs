...
if (!source.includes("const roundMoney = (value: number) => Math.round(value * 100) / 100;")) {
  const anchor = '  // ROOT ACCOUNTING RULE: reconciliation math comes from the canonical instrument ledger only.';
  if (!source.includes(anchor)) throw new Error("reconciliation rounding repair: root reconciliation block not found");

...
// Settlement available balance must use the canonical payment_instruments.current_balance cache.
// This cache is refreshed by DB accounting triggers and correctly includes fees/commissions,
// Quick Sale UPI collections, and settlement movements. Recomputing it from only `transactions`
// misses valid UPI collections and AEPS commission flows.
patch("components/finance/settlement-form-modal.tsx", (text) => {
...
// Account-level fallback remains for any legacy instrument without a balance cache.
patch("components/finance/settlement-form-modal.tsx", (text) => {

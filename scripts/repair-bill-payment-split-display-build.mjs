...
if (!source.includes("rowPaymentAllocations.length > 1")) {
  ...
  if (exprPos >= 0) {
    ...
    if (tdStart >= 0 && tdEnd >= 0) {
      ...
      console.log("Bill payment split display repair: payment cell not structurally replaceable; leaving existing source unchanged.");
    }
  } else {
    console.log("Bill payment split display repair: payment cell already transformed or layout changed; leaving existing source unchanged.");
  }
}

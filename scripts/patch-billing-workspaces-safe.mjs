...
const MARKER = "// BILLING_WORKSPACE_PATCH_SAFE_V1";
...
function patch(file, edits) {
  ...
  for (const [from, to] of edits) {
    if (!source.includes(from)) {
      throw new Error(`Billing patch anchor missing in ${file}: ${from.slice(0, 120)}`);
    }
...
patch("components/business/recharge-workspace.tsx", [
  [
  ...
  [
    '              ))}\n            </div>\n\n            {/* Plan Cards Slider */}',
    `              ))}\n            </div>\n\n${rechargeAmountPresets}            {/* Plan Cards Slider */}`,
  ],
  [
...
patch("components/business/utility-bill-workspace.tsx", [
  [
...
console.log("Billing workspace patch applied successfully.");

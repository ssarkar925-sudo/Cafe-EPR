...
replaceOnce(
  ...
  "settlement route catalog"
);

// 2. Credit-card pool label.
replaceOnce(
  '  upi_qr: "UPI QR",\n};',
...
// 3. Eligible instrument collections and route flags.
replaceOnce(
...
replaceOnce(
  ...
  "destination route flags"
);

// 4. Source account balance resolution. Canonical current_balance is authoritative when available.
replaceOnce(
  ...
  "source balance resolver"
);
replaceOnce(
  '  }, [open, sourceId, type, loadedPortals, merchantQrs, loadedAccounts, livePools, isSourceBank, isSourceWallet]);',
  '  }, [open, sourceId, type, loadedPortals, merchantQrs, loadedAccounts, livePools, isSourceBank, isSourceCash, isSourceWallet]);',
  "source balance dependencies"
);

// 5. Submit validation / labels.
replaceOnce(
...
replaceOnce(
  ...
  '    const routingTag = type === "bank_to_credit_card"\n      ? "[BANK → CREDIT CARD REPAYMENT]"\n      : type === "cash_to_credit_card"\n        ? "[CASH → CREDIT CARD REPAYMENT]"\n        : `[#${selected.from.toUpperCase()}→${selected.to.toUpperCase()}]`;',
  "routing tag"
);

if (source === original) throw new Error("Credit-card repayment patch made no changes");
fs.writeFileSync(path, source);
console.log("Patched settlement UI with credit-card repayment routes.");

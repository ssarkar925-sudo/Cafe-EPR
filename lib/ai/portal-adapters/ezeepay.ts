...
/**
 * Creates an EzeePay adapter from an explicitly learned selector map.
 * No selectors are guessed here: an unknown/changed portal fails closed.
 */
export function createEzeePayAdapter(selectors: EzeePaySelectors): EzeePayAdapter {
  return {
    providerName: "EzeePay",
    readOnly: true,
    async collect(page) {
      if (!selectors.historySelector || !selectors.rowSelectorTemplate.includes("{index}")) {
        return stop("workflow_not_learned", "EzeePay workflow selectors have not been taught yet.");
      }
      ...
      if (/\b(otp|one[- ]time password|pin|password|passcode|aadhaar|captcha|payment authorization)\b/i.test(beforeHistory)) {
        return stop("secret_requested", "EzeePay requested a secret or payment authorization; worker stopped.");
      }
      if (/\b(login|sign[ -]?in|mfa|verification code)\b/i.test(beforeHistory)) {
        return stop("login_required", "EzeePay requires authentication or verification; worker stopped.");
      }

      if ((await page.locatorCount(selectors.historySelector)) !== 1) {
        return stop("layout_changed", "EzeePay history control does not match the learned workflow.");
      }
      ...
      if (selectors.cashWithdrawalFilterSelector) {
        if ((await page.locatorCount(selectors.cashWithdrawalFilterSelector)) !== 1) {
          return stop("layout_changed", "EzeePay AEPS/cash-withdrawal filter no longer matches the learned workflow.");
        }
      ...
      for (let index = 1; index <= 500; index += 1) {
        ...
        if (errors.length) {
          return stop("ambiguous_transaction", `EzeePay row ${index} failed validation: ${errors.join(", ")}.`);
        }

...
export async function requireOwnerApproval(action: AgentAction, payload: Record<string, unknown>) {
  if (!isExecutableAction(action) || !isApprovalRequired(action)) throw new Error("This action is not executable through the approval gate.");
  ...
  const { data, error } = await supabase.from("ai_action_approvals").insert({ requested_by: userResult.user.id, action, status: "pending", request_payload: payload }).select("id, action, status, request_payload, created_at, expires_at").single();
...
export async function approveAction(approvalId: string, note?: string) {
  ...
  const { data, error } = await supabase.from("ai_action_approvals").update({ status: "approved", approved_by: userResult.user.id, approved_at: now, decision_note: note?.trim() || null }).eq("id", approvalId).eq("status", "pending").gt("expires_at", now).select("id, action, status, request_payload, created_at, expires_at, approved_at").single();
  if (error) throw new Error(error.message);
  return data;
}
...
export async function claimApprovedAction(approvalId: string) {
  ...
  const { data: userResult } = await supabase.auth.getUser();
  if (!userResult.user) throw new Error("Unauthorized");
  const { data, error } = await supabase.from("ai_action_approvals").update({ status: "executing" }).eq("id", approvalId).eq("status", "approved").select("id, action, status, request_payload, created_at, expires_at, approved_at").single();

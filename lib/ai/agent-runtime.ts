...
const TOOL_DECLARATIONS = [
  { name: "get_business_snapshot", description: "Read the current Cafe-EPR business snapshot: current-month P&L, customer receivables, low stock, and latest self-audit status. Use this before making claims about live business state.", parameters: { type: "object", properties: {} } },
  { name: "search_catalog", description: "Search active products and services by name. Use this when the owner asks about an item, price, stock, GST, HSN/SAC, or what can be sold.", parameters: { type: "object", properties: { query: { type: "string", description: "Product or service name to search for" } }, required: ["query"] } },
  { name: "search_customer", description: "Search active customers by name or phone and return current balance and basic account details. Do not invent a customer match.", parameters: { type: "object", properties: { query: { type: "string", description: "Customer name or phone" } }, required: ["query"] } },
  { name: "get_recent_transactions", description: "Read recent completed or pending Cafe-EPR service transactions. Useful for transaction status, recent activity, and operational questions.", parameters: { type: "object", properties: { limit: { type: "integer", description: "Number of recent transactions, maximum 25" } } } },
  { name: "get_audit_status", description: "Read the latest Cafe-EPR self-audit run and its findings. Use for integrity, discrepancy, anomaly, or audit questions.", parameters: { type: "object", properties: {} } },
  { name: "get_whatsapp_status", description: "Perform a live WhatsApp provider health check. Use for WhatsApp connection, gateway, error, disconnect, or reconnect questions.", parameters: { type: "object", properties: {} } },
  { name: "get_learning_workflows", description: "Read the owner's active or recently drafted learned AI workflows so the agent can follow known procedures without guessing.", parameters: { type: "object", properties: {} } },
];

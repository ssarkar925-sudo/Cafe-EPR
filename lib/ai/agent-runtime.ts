...
function monthRange() {
  ...
  const inclusiveDate = new Date(next.getTime() - 86400000);
  ...
  switch (call.name) {
    case "get_business_snapshot": {
      ...
      return {
        ...
        receivables: { totalTop10: money(customers.reduce((sum: number, c: any) => sum + safeNumber(c.balance), 0)), topCustomers: customers.map((c: any) => ({ name: c.name, phone: c.phone || null, balance: safeNumber(c.balance) })) },
...
function normalizeHistory(history: AgentHistoryItem[] | undefined): AgentHistoryItem[] {
  return (Array.isArray(history) ? history : []).filter((item) => item && (item.role === "user" || item.role === "assistant") && typeof item.content === "string").map((item) => ({ role: item.role, content: item.content.trim().slice(0, 4000) })).filter((item) => item.content).slice(-MAX_HISTORY);
}
...
async function callGemini(apiKey: string, model: string, body: Record<string, unknown>) {
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": apiKey }, body: JSON.stringify(body), signal: AbortSignal.timeout(30000) });
...
export async function runIntelligentAgent({ apiKey, message, history, systemInstruction, supabase, userId }: { apiKey: string; message: string; history?: AgentHistoryItem[]; systemInstruction: string; supabase: SupabaseClient<any, any, any>; userId: string; }) {
  ...
  for (let round = 0; round <= MAX_TOOL_ROUNDS; round += 1) {
    ...
    for (const model of getModelCandidates()) {
      try { result = await callGemini(apiKey, model, { ...baseBody, contents }); if (result.ok) break; lastError = result.error; } catch (error) { lastError = error instanceof Error ? error.message : lastError; }
    }

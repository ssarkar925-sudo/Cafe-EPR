...
function getFinancialIdempotencyKey(operation: string, args: Record<string, unknown>): { cacheKey: string; key: string } {
  ...
  if (!key && typeof sessionStorage !== "undefined") {
    try { key = sessionStorage.getItem(cacheKey) || undefined; } catch { /* best effort */ }
  }
  if (!key) {
    key = newIdempotencyKey();
    idempotencyKeys.set(cacheKey, key);
    if (typeof sessionStorage !== "undefined") {
      try { sessionStorage.setItem(cacheKey, key); } catch { /* best effort */ }
    }
  }
  return { cacheKey, key };
}
...
function clearFinancialIdempotencyKey(cacheKey: string) {
  ...
  if (typeof sessionStorage !== "undefined") {
    try { sessionStorage.removeItem(cacheKey); } catch { /* best effort */ }
  }
}
  ...
  return new Proxy(client, {
    get(target, prop, receiver) {
      ...
      return (functionName: string, args?: Record<string, unknown>, ...rest: unknown[]) => {
        ...
        if (!FINANCIAL_IDEMPOTENT_RPCS.has(functionName)) return directRpc(functionName, args, ...rest);

        const requestArgs: Record<string, unknown> = { ...(args ?? {}) };
        let cacheKey: string | null = null;
        const explicitKey = typeof requestArgs.p_idempotency_key === "string" ? requestArgs.p_idempotency_key.trim() : "";
        if (!explicitKey) {
...
export function clearClientAuthCookies() {
  ...
  try {
    ...
    for (const cookie of cookies) {
      ...
      if (name.startsWith("sb-") || name.includes("auth-token") || name.includes("supabase") || name.includes("refresh-token") || name.includes("access-token")) {
        document.cookie = `${name}=; path=/; max-age=0; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=None; Secure`;
        document.cookie = `${name}=; path=/; max-age=0; expires=Thu, 01 Jan 1970 00:00:00 GMT`;
      }
    }
  } catch { /* ignore cookie clearing errors */ }
  try {
    if (typeof localStorage !== "undefined") {
      const toRemove: string[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && (key.startsWith("sb-") || key.includes("supabase") || key.includes("auth-token") || key.includes("refresh-token"))) toRemove.push(key);
      }
...
function setupBrowserAuthErrorHandlers(client: ReturnType<typeof createBrowserClient>) {
  ...
  if (!rejectionListenerInstalled) {
    ...
    window.addEventListener("unhandledrejection", (event) => {
      ...
      const isAuthError = reason?.__isAuthError === true || reason?.name === "AuthApiError" || msg.includes("Invalid Refresh Token") || msg.includes("Refresh Token Not Found");
      if (isAuthError && (msg.includes("Refresh Token") || msg.includes("refresh_token") || reason?.__isAuthError)) {
        event.preventDefault();
        clearClientAuthCookies();
        if (window.location.pathname !== "/login" && !window.location.pathname.startsWith("/login")) window.location.href = "/logout?reason=expired";
      }
    });
  }
}
        ...
        return new Proxy(builder, {
          get(query, queryProp, queryReceiver) {
            ...
            return (columns?: string, ...rest: any[]) => {
              const rewrittenColumns = typeof columns === "string" ? columns.split(",").map((column) => column.trim() === "remarks" ? "description" : column.trim()).join(",") : columns;
              return (query as any).select(rewrittenColumns, ...rest).neq("ref_type", "settlement");
            };
          },
        });
      };
...
export function createClient() {
  ...
  if (typeof window === "undefined") return createBrowserClient(url, anonKey, { cookieOptions: { path: "/", sameSite: "none", secure: true } });
  if (!browserClient) {
    const rawClient = createBrowserClient(url, anonKey, { cookieOptions: { path: "/", sameSite: "none", secure: true } });

...
export async function POST(req: Request) {
  try {
    ...
    const provider: WhatsAppConfig["provider"] = baseProvider && baseProvider !== "off" ? baseProvider : secrets?.meta_access_token ? "meta" : secretProvider || baseProvider || "off";
    ...
    if (provider === "meta") {
      ...
      const displayPhone = String(base.meta_display_phone_number || "").trim();
      let phoneId = String(secrets?.meta_phone_number_id || "").trim();
      ...
      await db.from("whatsapp_templates").update({ config: { ...base, meta_waba_id: wabaId, meta_display_phone_number: resolved.phone || displayPhone }, updated_at: new Date().toISOString() }).eq("id", "default");
    ...
    const config: WhatsAppConfig = { ...base, provider, automations: base.automations, meta_phone_number_id: secrets?.meta_phone_number_id, meta_access_token: secrets?.meta_access_token, gateway_api_key: secrets?.gateway_api_key, ultramsg_token: secrets?.ultramsg_token, ultramsg_instance_id: secrets?.ultramsg_instance_id };
    const result = await sendWhatsAppViaConfig(recipient, "CafeERP WhatsApp connection test. If you received this message, the configured sender is working.", config);
    if (!result.success) return NextResponse.json(result, { status: result.status || 400 });

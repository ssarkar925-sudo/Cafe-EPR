...
export async function POST(request: Request) {
  ...
  if (STOCK_ALERT_PATTERN.test(message)) {
    try {
      const { data: products } = await supabase.from("products").select("id,name,stock_qty,reorder_level,unit").eq("is_active", true);
      const lowStock = (products || []).filter((p) => Number(p.stock_qty || 0) <= Number(p.reorder_level || 0));
      if (lowStock.length === 0) {
        return NextResponse.json({ message: "✓ All catalog products are currently healthy and well above reorder thresholds.", mode: "inventory-report", canExecute: false, approvalRequired: false });
      }
      const list = lowStock.map((p) => `- ${p.name}: Stock ${p.stock_qty} ${p.unit || "units"} (Reorder Level: ${p.reorder_level})`).join("\n");
      return NextResponse.json({ message: `⚠️ Inventory Alert: ${lowStock.length} item(s) need reordering:\n\n${list}`, mode: "inventory-report", canExecute: false, approvalRequired: false });
    } catch (err) {}
  }

  if (DUES_PATTERN.test(message)) {
    try {
      const { data: customers } = await supabase.from("customers").select("id,name,phone,balance").gt("balance", 0).order("balance", { ascending: false });
      if (!customers || customers.length === 0) {
        return NextResponse.json({ message: "✓ All customer accounts are fully settled. No Khata receivables pending.", mode: "dues-report", canExecute: false, approvalRequired: false });
      }
      const totalDues = customers.reduce((sum, c) => sum + Number(c.balance || 0), 0);
      const list = customers.slice(0, 10).map((c) => `- ${c.name} (${c.phone || "No phone"}): ${formatInr(Number(c.balance))}`).join("\n");
      return NextResponse.json({ message: `📋 Customer Khata Dues (${customers.length} account(s) owing ${formatInr(totalDues)} total):\n\n${list}\n\nUse the Customers module to send 1-click WhatsApp payment reminders with your UPI QR code.`, mode: "dues-report", canExecute: false, approvalRequired: false });
    } catch (err) {}
  }
  ...
  for (const model of models) {
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
        ...
        body: JSON.stringify({ systemInstruction: { parts: [{ text: systemInstruction }] }, contents: [{ role: "user", parts: [{ text: message }] }] }),
      });
  ...
  if (!data?.candidates?.[0]?.content?.parts) {
    console.error("AI agent Gemini request failed", lastError);

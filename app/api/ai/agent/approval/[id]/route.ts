...
type SaleLine = { product_id: string | null; service_id: string | null; description: string; qty: number; rate: number; cost_price: number; hsn_sac: string | null; gst_rate: number; tax_treatment: string; };
...
async function restoreApproved(id: string, note: string) { const supabase = await createClient(); await supabase.from("ai_action_approvals").update({ status: "approved", decision_note: note }).eq("id", id).eq("status", "executing"); }
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (approval.action === "repair_whatsapp") {
      ...
      if (!after.connected) { await restoreApproved(id, `Repair attempted but verification still failed: ${after.error || after.status}.`); throw new Error(`Repair was attempted, but WhatsApp is still unhealthy: ${after.error || after.status}. No further automatic changes were made.`); }
    ...
    const [{ data: products }, { data: services }] = await Promise.all([supabase.from("products").select("id,name,sale_price,cost_price,stock_qty,hsn_code,gst_rate").in("id", ids), supabase.from("services").select("id,name,sale_price,cost_price,sac_code,gst_rate").in("id", ids)]);
    const catalog = [...(products ?? []).map((p: any) => ({ ...p, kind: "product" })), ...(services ?? []).map((s: any) => ({ ...s, kind: "service" }))];
    const lines: SaleLine[] = itemsPayload.map((item: any) => { const current = catalog.find((x: any) => x.id === item.id && x.kind === item.kind); if (!current) throw new Error(`Item no longer exists: ${item.name || item.id}`); const currentRate = Number(current.sale_price); if (Math.abs(currentRate - Number(item.rate)) > 0.001) throw new Error(`${current.name} price changed from ₹${Number(item.rate).toFixed(2)} to ₹${currentRate.toFixed(2)}. A new approval is required.`); const qty = Math.floor(Number(item.qty)); if (qty <= 0) throw new Error(`Invalid quantity for ${current.name}.`); if (current.kind === "product" && Number(current.stock_qty) < qty) throw new Error(`${current.name} no longer has enough stock.`); return { product_id: current.kind === "product" ? current.id : null, service_id: current.kind === "service" ? current.id : null, description: current.name, qty, rate: currentRate, cost_price: Number(current.cost_price ?? 0), hsn_sac: current.kind === "product" ? current.hsn_code ?? null : current.sac_code ?? null, gst_rate: Number(current.gst_rate ?? 0), tax_treatment: Number(current.gst_rate ?? 0) > 0 ? "taxable" : "non_gst" }; });
    ...
    const expectedTotal = Number(payload.expected_total); if (Math.abs(gst.invoiceTotal - expectedTotal) > 0.01) throw new Error(`Sale total changed from ₹${expectedTotal.toFixed(2)} to ₹${gst.invoiceTotal.toFixed(2)}. A new approval is required.`);
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to approve/execute action" }, { status: 403 }); }
}

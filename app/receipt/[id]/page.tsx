        ...
        <div className="font-mono text-xs leading-relaxed text-slate-900">
              ...
              return (
                <div key={it.id || idx} className="text-[11px]">
                  ...
                  <div className="flex justify-between text-slate-600 pl-2">
                    <span>{Number(it.qty)} x {money(it.rate)}</span>
          ...
          <div className="space-y-0.5 text-[11px]">
            ...
            {Number(invoice.discount || 0) > 0 && (
              <div className="flex justify-between text-emerald-700 font-bold"><span>Discount:</span><span>- {money(invoice.discount)}</span></div>
            )}
            <div className="flex justify-between text-sm font-black pt-1 border-t border-slate-300"><span>TOTAL:</span><span>{money(invoice.total)}</span></div>
            <div className="flex justify-between font-bold pt-0.5"><span>Amount Paid:</span><span className="text-emerald-700">{money(invoice.paid)}</span></div>
            {Number(invoice.due || 0) > 0 && (
              <div className="flex justify-between font-bold text-amber-700 border-t border-slate-200 pt-0.5"><span>Balance Due:</span><span>{money(invoice.due)}</span></div>
            )}
          </div>
            ...
            <>
              ...
              <div className="space-y-0.5 text-[10px]">
                ...
                {paymentsRows.map((p, idx) => (
                  <div key={idx} className="flex justify-between pl-2"><span>• {String(p.method).toUpperCase()}</span><span>{money(p.amount)}</span></div>
                ))}
              </div>
            </>
          )}
          ...
          {qrDataUrl && (
            <>
              ...
              <div className="flex flex-col items-center justify-center text-center py-1 bg-slate-50/70 rounded-xl p-2.5">
                <img src={qrDataUrl} alt="Scan to Pay via UPI" className="h-28 w-28 object-contain rounded-lg border border-slate-300 bg-white p-1" />
                <p className="mt-1.5 font-black text-[11px] tracking-wider text-slate-900">SCAN &amp; PAY VIA UPI</p>
                <p className="text-[10px] font-bold text-emerald-800">Amount: {money(targetAmount)}</p>

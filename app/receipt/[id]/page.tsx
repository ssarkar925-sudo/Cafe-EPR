...
export default async function ReceiptPage({
  ...
  // Public receipts must use the opaque invoice UUID. Invoice numbers are
  // sequential/business identifiers and must never be an unauthenticated lookup key.
  ...
  const upiString = isDue && upiId
    ? generateUpiString({ upiId, name: settings?.shop_name || "Shop", amount: targetAmount, note: "Inv " + invoice.invoice_number })
    : "";
      ...
      <div className="mx-auto max-w-[340px] rounded-2xl border border-slate-200 bg-white p-5 shadow-lg print:max-w-none print:rounded-none print:border-none print:p-0 print:shadow-none">
        ...
        <div className="font-mono text-xs leading-relaxed text-slate-900">
          <div className="text-center">
            <p className="text-base font-black tracking-tight">{settings?.shop_name || "Sarkar Communication"}</p>
            ...
            <p className="text-[11px]">{settings?.phone && <span>Ph: {settings.phone}</span>}{settings?.phone && settings?.email && <span> · </span>}{settings?.email && <span>{settings.email}</span>}</p>
          ...
          <div className="flex justify-between items-center text-[11px]"><span className="font-bold">RECEIPT / BILL</span><span className="font-bold font-mono">#{invoice.invoice_number}</span></div>
          ...
          <div className="py-1 space-y-1">
            {itemsRows.map((it, idx) => { const name = it.products?.name || it.services?.name || it.description || "Item"; return <div key={it.id || idx} className="text-[11px]"><div className="font-medium text-slate-900">{name}</div><div className="flex justify-between text-slate-600 pl-2"><span>{Number(it.qty)} x {money(it.rate)}</span><span className="font-bold text-slate-900">{money(it.amount)}</span></div></div>; })}
            {itemsRows.length === 0 && <p className="py-2 text-center text-slate-400">No items recorded</p>}
          </div>
          <div className="my-2 border-t-2 border-dashed border-slate-400" />
          ...
          {paymentsRows.length > 0 && <><div className="my-2 border-t border-dashed border-slate-300" /><div className="space-y-0.5 text-[10px]"><span className="font-bold text-slate-600">TENDERED:</span>{paymentsRows.map((p, idx) => <div key={idx} className="flex justify-between pl-2"><span>• {String(p.method).toUpperCase()}</span><span>{money(p.amount)}</span></div>)}</div></>}
          {qrDataUrl && <><div className="my-2.5 border-t-2 border-dashed border-slate-400" /><div className="flex flex-col items-center justify-center text-center py-1 bg-slate-50/70 rounded-xl p-2.5"><img src={qrDataUrl} alt="Scan to Pay via UPI" className="h-28 w-28 object-contain rounded-lg border border-slate-300 bg-white p-1" /><p className="mt-1.5 font-black text-[11px] tracking-wider text-slate-900">SCAN &amp; PAY VIA UPI</p><p className="text-[10px] font-bold text-emerald-800">Amount: {money(targetAmount)}</p>{upiId && <p className="text-[9px] font-mono text-slate-600 truncate max-w-full">UPI: {upiId}</p>}<p className="text-[8px] text-slate-500">Google Pay · PhonePe · Paytm · BHIM</p></div></>}

...
export default async function ReceiptA4Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (invResult.data) {
    ...
    if (invoice.customer_id) {
      ...
      // Customer visibility is role-restricted in the ERP. A receipt must still render
      // when the authenticated user can read the invoice but not the customer master.
  ...
  // settings is back-office protected; receipt rendering uses safe defaults when it is not visible.

...
export default async function ReceiptA4Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (invoiceRow) {
    ...
    // Historical Quick Sale records are still valid financial documents. Keep
    ...
    payments = quickPayments && quickPayments.length > 0 ? quickPayments : [{ method: quickSale.payment_method || "cash", amount: quickSale.amount }];
  }
  ...
  const upiString = isDue && upiId ? generateUpiString({ upiId, name: settings?.shop_name || "Shop", amount: targetAmount, note: "Inv " + invoice.invoice_number }) : "";

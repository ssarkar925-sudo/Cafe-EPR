...
export default async function ReceiptA4Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Receipt rendering is an authenticated read operation. Do not depend on a
  // Supabase service-role secret in Cloudflare; the production Worker must be
  // able to render receipts with the user's normal authenticated session.
  ...
  if (invoiceResult.data) {
    ...
    const [itemsResult, paymentsResult] = await Promise.all([
      supabase
        ...
        .select("*, products(name, code, unit), services(name)")
        .eq("quick_sale_id", id),
      supabase
        .from("payments")
        .select("method, amount, received_at")
    ...
    invoice = {
      id: quickSale.id,
      invoice_number: quickSale.sale_number,
      invoice_date: quickSale.sale_date,
      subtotal: quickSale.amount,
      discount: 0,
      taxable_amount: quickSale.amount,
      cgst: 0,
      sgst: 0,
      igst: 0,
      total: quickSale.amount,
    ...
    <main className="min-h-screen bg-slate-100 p-4 print:bg-white print:p-0">
      ...
      <section className="mx-auto max-w-[820px] rounded-2xl border border-slate-200 bg-white p-6 shadow-lg print:max-w-none print:rounded-none print:border-none print:p-0 print:shadow-none">
        <A4Actions
          ...
          receiptUrl={`/receipt/${invoice.id}`}
        />

        <header className="flex items-start justify-between gap-6 border-b border-slate-300 pb-4">
          <div>
            <h1 className="text-xl font-black text-slate-900">{settings?.shop_name || "Sarkar Communication"}</h1>
            <p className="text-xs font-bold uppercase tracking-wider text-blue-700">Tax Invoice</p>
            {settings?.address && <p className="mt-1 text-xs text-slate-600">{settings.address}</p>}
            <p className="text-xs text-slate-600">{settings?.phone || ""}{settings?.email ? ` · ${settings.email}` : ""}</p>
            {(settings?.gstin || settings?.tax_id) && <p className="text-xs font-bold">GSTIN / Tax ID: {settings.gstin || settings.tax_id}</p>}
          </div>
          <div className="text-right text-xs text-slate-700">
            <p className="font-mono text-sm font-black">#{invoice.invoice_number}</p>
            <p>Date: {invoice.invoice_date}</p>
            <p className="mt-1 font-bold uppercase">{invoice.status || "completed"}</p>
          </div>
        </header>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <p className="font-bold text-slate-900">{invoice.customers?.name || "Walk-in Customer"}</p>
            {invoice.customers?.phone && <p className="text-xs text-slate-600">Phone: {invoice.customers.phone}</p>}
            {invoice.customers?.address && <p className="text-xs text-slate-600">{invoice.customers.address}</p>}
            {invoice.customer_gstin && <p className="text-xs font-bold">GSTIN: {invoice.customer_gstin}</p>}
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Settlement</p>
            <p>Payment: {paymentsRows.length ? paymentsRows.map((p) => String(p.method).toUpperCase()).join(", ") : "CASH"}</p>
            <p>Paid: <b>{money(invoice.paid)}</b></p>
            <p>Due: <b>{money(invoice.due)}</b></p>
          </div>
        </div>

        <table className="mt-5 w-full text-xs">
          <thead><tr className="border-y border-slate-900 bg-slate-900 text-white"><th className="p-2 text-left">#</th><th className="p-2 text-left">Item</th><th className="p-2 text-center">Qty</th><th className="p-2 text-right">Rate</th><th className="p-2 text-right">Amount</th></tr></thead>
          <tbody>
            {itemsRows.map((it, index) => {
              const name = it.products?.name || it.services?.name || it.description || "Item";
              return <tr key={it.id || index} className="border-b border-slate-100"><td className="p-2">{index + 1}</td><td className="p-2 font-semibold">{name}{it.products?.code ? ` [${it.products.code}]` : ""}</td><td className="p-2 text-center">{Number(it.qty)}</td><td className="p-2 text-right">{money(it.rate)}</td><td className="p-2 text-right font-bold">{money(it.amount)}</td></tr>;
            })}
          </tbody>
        </table>
        ...
        <div className="mt-5 grid grid-cols-2 gap-5">
          <div>
            ...
            <p className="text-xs font-bold">{numberToWordsInr(Number(invoice.total || 0))}</p>
            {qrDataUrl && <div className="mt-3 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-2"><img src={qrDataUrl} alt="UPI payment QR" className="h-16 w-16" /><div className="text-xs"><b>Scan & Pay via UPI</b><p>{upiId}</p><p>Amount: {money(qrAmount)}</p></div></div>}
          </div>
          <div className="rounded-xl border border-slate-300 p-3 text-xs">
            <div className="flex justify-between"><span>Subtotal</span><b>{money(invoice.subtotal)}</b></div>
            {Number(invoice.discount || 0) > 0 && <div className="flex justify-between"><span>Discount</span><b>- {money(invoice.discount)}</b></div>}
            {Number(invoice.cgst || 0) > 0 && <div className="flex justify-between"><span>CGST</span><span>{money(invoice.cgst)}</span></div>}
            {Number(invoice.sgst || 0) > 0 && <div className="flex justify-between"><span>SGST</span><span>{money(invoice.sgst)}</span></div>}
            {Number(invoice.igst || 0) > 0 && <div className="flex justify-between"><span>IGST</span><span>{money(invoice.igst)}</span></div>}
            <div className="mt-2 flex justify-between border-t-2 border-slate-900 pt-2 text-sm font-black"><span>Grand Total</span><span>{money(invoice.total)}</span></div>
            <div className="mt-1 flex justify-between"><span>Paid</span><b>{money(invoice.paid)}</b></div>
            <div className="flex justify-between"><span>Balance Due</span><b>{money(invoice.due)}</b></div>
          </div>
        </div>

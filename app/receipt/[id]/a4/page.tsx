      ...
      <section className="mx-auto max-w-[820px] rounded-2xl border border-slate-200 bg-white p-6 shadow-lg print:max-w-none print:rounded-none print:border-none print:p-0 print:shadow-none">
        ...
        <header className="flex items-start justify-between gap-6 border-b border-slate-300 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-600">{settings.phone || ""}{settings.phone && settings.email ? " · " : ""}{settings.email || ""}</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs">
            ...
            <p>Payment: {payments.length ? payments.map((p) => String(p.method).toUpperCase()).join(", ") : "CASH"}</p>
        ...
        <table className="mt-5 w-full text-xs">
          ...
          <tbody>
            {items.map((it, index) => {
              const itemName = it.description || it.item_name || it.products?.name || it.services?.name || "Item";
              return (
                <tr key={it.id || index} className="border-b border-slate-100">
                  <td className="p-2">{index + 1}</td>
                  <td className="p-2 font-semibold">{itemName}</td>
                  <td className="p-2 text-center">{Number(it.qty)}</td>
                  <td className="p-2 text-right">{money(it.rate)}</td>
                  <td className="p-2 text-right font-bold">{money(it.amount)}</td>
                </tr>
              );
            })}
            {!items.length && <tr><td colSpan={5} className="p-4 text-center text-slate-400">No line items recorded</td></tr>}
          </tbody>
        </table>
          ...
          <div>
            ...
            {qrDataUrl && (
              <div className="mt-3 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-2">
                <img src={qrDataUrl} alt="UPI payment QR" className="h-16 w-16" />
                <div className="text-xs">
                  <b>Scan &amp; Pay via UPI</b>
                  <p>{upiId}</p>
                  <p>Amount: {money(qrAmount)}</p>
                </div>
              </div>
            )}
          </div>

...
export default async function ProfitLossPage() {
  ...
  const invoiceIds = new Set((invoices ?? []).map((x) => x.invoice_id));
  ...
  const csv = [["Line Item", "Amount (INR)"], ...rows.map(([label, value]) => [label, Number(value).toFixed(2)]), ["Net Margin (%)", `${margin.toFixed(2)}%`]]
  ...
  return (
    <div className="space-y-6 pb-12" id="profit-loss-report">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 dark:border-white/10 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400"><Link href="/reports" className="hover:text-blue-600 dark:hover:text-blue-400">Reports &amp; Tax Hub</Link><ChevronRight className="h-3.5 w-3.5 text-slate-400" /><span className="font-bold text-slate-900 dark:text-white">Profit &amp; Loss Statement</span></div>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl dark:text-white">Profit &amp; Loss Statement</h1>
          <p className="mt-1 text-xs text-slate-600 sm:text-sm dark:text-slate-400">Deterministic P&amp;L accounting: Revenue, returns, actual invoice-item COGS, service fees, and operating expenses.</p>
        </div>

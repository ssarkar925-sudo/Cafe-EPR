...
export const metadata = {
  title: "Inventory | CyberCafe ERP",
  description: "Stock overview, alerts and inventory operations",
};
...
export default async function InventoryPage() {
  ...
  const rows = (products ?? []) as Array<{
    id: string; name: string; code: string | null; stock_qty: number | null;
    cost_price: number | null; reorder_level: number | null;
  }>;
      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900 sm:p-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400"><span className="h-1.5 w-1.5 rounded-full bg-current" /> Inventory control</div>
              <h2 className="text-3xl font-black tracking-tight text-slate-950 dark:text-white">Stock, under control.</h2>
              <p className="mt-2 max-w-2xl text-sm text-slate-500 dark:text-slate-400">A single operational view for stock value, replenishment risk and audited inventory activity.</p>
            </div>
            <Link href="/inventory/movements" className="inline-flex w-fit items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-slate-900/10 transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">Open Stock Journal <span>→</span></Link>
          </div>
        </section>

        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ["Active Products", rows.length.toLocaleString(), "items in catalog", "blue"],
            ["Units on Hand", totalUnits.toLocaleString(), "physical units", "indigo"],
            ["Stock Value", inr(stockValue), "at current cost", "emerald"],
            ["Replenishment", (low.length + out.length).toLocaleString(), `${low.length} low · ${out.length} out`, "amber"],
            ...
            <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900">
              <div className={`mb-4 h-2 w-12 rounded-full bg-${tone}-500`} />
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">{label}</p>
              <p className="mt-1 truncate text-2xl font-black text-slate-950 dark:text-white">{value}</p>

...
export default function ProductsClient({ initialProducts, categories, embedded = false }: { initialProducts: Product[]; categories: CategoryRef[]; embedded?: boolean }) {
  ...
  const statCards = [
    { label:"Total Products", value:String(stats.total), icon:"M3 6h18M3 12h18M3 18h18", grad:"from-blue-500 to-indigo-600", glow:"card-glow-indigo", valColor:"text-indigo-700 dark:text-indigo-300" },
    { label:"Active", value:String(stats.active), icon:"M22 11.08V12a10 10 0 1 1-5.93-9.14M22 4 12 14l-3-3", grad:"from-emerald-500 to-teal-600", glow:"card-glow-emerald", valColor:"text-emerald-700 dark:text-emerald-300" },
    { label:"Low Stock", value:String(stats.low), icon:"M12 9v4M12 17h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z", grad:stats.low>0?"from-rose-500 to-pink-600":"from-slate-400 to-slate-500", glow:stats.low>0?"card-glow-rose":"card-glow-indigo", valColor:stats.low>0?"text-rose-700 dark:text-rose-300":"text-slate-700 dark:text-slate-300" },
    { label:"Stock Value", value:inr(stats.stockValue), icon:"M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6", grad:"from-amber-500 to-orange-600", glow:"card-glow-amber", valColor:"text-amber-700 dark:text-amber-300" },
  ];
      ...
      {statCards.map((c) => (
        <div
          ...
          onClick={() => {
            ...
            else if (c.label === "Total Products") { setStatus("all"); setCat("all"); setQ(""); }
            else setStatus("all");
          }}
          className={`bento-surface relative cursor-pointer overflow-hidden rounded-2xl border p-5 transition-all duration-200 hover:-translate-y-0.5 ${c.glow}`}
        >

...
export default function AICommandCenter() {
  ...
  const taskRef = useRef(DEFAULT_TASK);
  ...
  const run = useCallback(async (prepareRepair = false, overrideTask?: string) => {
    const mission = (overrideTask ?? taskRef.current).trim();
  ...
  async function approve() {
    ...
    try {
      ...
      busyRef.current = false;
      setBusy(false);
      await run(false, taskRef.current);
    } catch (error) {
      busyRef.current = false;
      setBusy(false);
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-indigo-200 bg-white shadow-sm dark:border-indigo-500/20 dark:bg-slate-900">
      <div className="bg-slate-950 px-5 py-5 text-white sm:px-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="inline-flex items-center gap-2 rounded-full border border-indigo-400/30 bg-indigo-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-indigo-200"><span className="h-2 w-2 animate-pulse rounded-full bg-indigo-400" /> AI Command Center</div><h2 className="mt-2 text-2xl font-black tracking-tight">Plan → Check → Detect → Prepare → Approve → Verify</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-slate-300">Cafe AI can coordinate multiple modules in one mission. Read-only analysis runs without approval; consequential repairs are staged and require explicit owner approval.</p></div><div className="grid grid-cols-2 gap-2 text-center text-[10px] font-black uppercase"><div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2"><div className="text-indigo-300">Modules</div><div className="mt-1 text-lg">{result?.checks.length ?? "—"}</div></div><div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2"><div className="text-amber-300">Attention</div><div className="mt-1 text-lg">{attention}</div></div></div></div></div>
      <div className="p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row"><input value={task} onChange={(event) => { setTask(event.target.value); taskRef.current = event.target.value; }} onKeyDown={(event) => { if (event.key === "Enter") void run(false); }} className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-800 outline-none focus:border-indigo-400 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100" placeholder="Tell Command Center what outcome you want…" /><button type="button" onClick={() => void run(false)} disabled={busy} className="rounded-2xl bg-indigo-600 px-5 py-3 text-xs font-black text-white shadow-sm hover:bg-indigo-700 disabled:opacity-50">{busy ? "Working…" : "Run mission"}</button></div>

      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 active:translate-y-0.5 dark:border-white/10 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
          >
          ...
          <button
            ...
            className="btn-3d-tactile-primary inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold text-white shadow-md shadow-blue-500/20 active:translate-y-0.5 disabled:opacity-50"
          >
      ...
      <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-6 text-white shadow-xl">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-6 self-start md:self-auto">
            <div className="card-glow-emerald flex flex-col items-center justify-center rounded-2xl bg-white/10 p-5 border border-white/10 backdrop-blur-md min-w-[140px] shadow-sm">
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-5 gap-3 border-t border-white/10 pt-4 text-xs">
          <div className="card-glow-indigo rounded-2xl border border-indigo-500/20 bg-white/5 p-3 backdrop-blur-md transition hover:-translate-y-0.5">
            <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Total Invariants</p>
            <p className="text-base font-black text-white mt-1">{total} Checks</p>
          </div>
          <div className="card-glow-emerald rounded-2xl border border-emerald-500/20 bg-white/5 p-3 backdrop-blur-md transition hover:-translate-y-0.5">
            <p className="text-emerald-400 text-[10px] uppercase font-bold tracking-wider">Passed (₹0.00)</p>
            <p className="text-base font-black text-emerald-300 mt-1">{passed} Subsystems</p>
          </div>
          <div className="card-glow-amber rounded-2xl border border-amber-500/20 bg-white/5 p-3 backdrop-blur-md transition hover:-translate-y-0.5">
            <p className="text-amber-400 text-[10px] uppercase font-bold tracking-wider">Warnings</p>
            <p className="text-base font-black text-amber-300 mt-1">{warnings}</p>
          </div>
          <div className="card-glow-rose rounded-2xl border border-rose-500/20 bg-white/5 p-3 backdrop-blur-md transition hover:-translate-y-0.5">
            <p className="text-rose-400 text-[10px] uppercase font-bold tracking-wider">Failures</p>
            <p className="text-base font-black text-rose-300 mt-1">{failed}</p>
          </div>
          <div className="card-glow-rose rounded-2xl border border-red-500/20 bg-white/5 p-3 backdrop-blur-md transition hover:-translate-y-0.5">
            <p className="text-red-400 text-[10px] uppercase font-bold tracking-wider">Critical</p>
            <p className="text-base font-black text-red-400 mt-1">{critical}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto border-b border-slate-200 dark:border-white/10 pb-2">
        {SECTIONS.map((sec) => (
          <button
            ...
            className={`rounded-xl px-3.5 py-1.5 text-xs font-bold whitespace-nowrap transition-all duration-200 active:translate-y-0.5 ${
              activeSection === sec.id
                ? "bg-slate-900 text-white shadow-md ring-1 ring-black/5 dark:bg-blue-600 dark:text-white"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredFindings.map((f) => {
              const cardGlow = f.status === "PASS" ? "card-glow-emerald" : f.status === "WARNING" ? "card-glow-amber" : "card-glow-rose";
              return (
                <div
                  key={f.id}
                  onClick={() => handleSelectFinding(f)}
                  className={`group relative cursor-pointer overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4.5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:border-indigo-300 space-y-3 flex flex-col justify-between ${cardGlow}`}
                >
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold ring-1 ${
                          f.status === "PASS"
                            ...
                            : "bg-red-100 text-red-950 ring-red-600/30 font-black animate-pulse"
                        }`}
                      >
                    ...
                    <h3 className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors mt-2">
                  ...
                  <div className="pt-2.5 border-t border-slate-100 space-y-1.5 text-xs">
                    <div className="flex justify-between items-center text-slate-600">
                      <span className="text-[11px]">Variance:</span>

...
type Finding = { id: string; severity: "critical" | "high" | "medium" | "info"; area: string; title: string; evidence: string; recommendation: string; autoFixable: boolean; repairBlocker?: string; repairKind?: string; actionUrl?: string };
    ...
    <section className="rounded-3xl border border-amber-200 bg-amber-50/70 p-4 shadow-sm dark:border-amber-500/20 dark:bg-amber-500/5">
      ...
      {findings.length > 0 ? <div className="mt-4 space-y-3">{findings.slice(0, 5).map((finding) => <div key={finding.id} className="rounded-2xl border border-white/70 bg-white/90 p-4 dark:border-white/10 dark:bg-slate-900/80">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {finding.autoFixable && finding.repairKind && <button type="button" onClick={() => void prepareRepair()} disabled={busy || !!approval} className="rounded-xl bg-amber-600 px-3 py-2 text-[11px] font-black text-white disabled:opacity-50">{approval ? "Solution ready below" : "Prepare executable AI fix"}</button>}
          {finding.actionUrl && <button type="button" onClick={() => window.open(finding.actionUrl, "_blank", "noopener,noreferrer") } className="rounded-xl border border-blue-300 bg-white px-3 py-2 text-[11px] font-black text-blue-700 hover:bg-blue-50 dark:border-blue-500/30 dark:bg-slate-900 dark:text-blue-300">Open WhatsApp Gateway & Scan QR</button>}
        </div>
        {!finding.autoFixable && !finding.actionUrl && <div className="mt-3 text-[10px] font-bold text-slate-500">AI will not invent credentials, overwrite business data, or call an undocumented provider endpoint just to appear automated.</div>}

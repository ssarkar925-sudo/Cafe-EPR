...
export default function AISelfHealingBridge() {
  ...
  return (
    <section className="rounded-3xl border border-amber-200 bg-amber-50/70 p-4 shadow-sm dark:border-amber-500/20 dark:bg-amber-500/5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 animate-pulse rounded-full bg-amber-500" /><h2 className="text-base font-black text-slate-900 dark:text-white">AI Application Guardian</h2><span className="rounded-full border border-amber-300 bg-white px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-amber-700 dark:border-amber-500/30 dark:bg-slate-900 dark:text-amber-300">Diagnose → Ask → Repair → Verify</span></div><p className="mt-1 text-xs text-slate-600 dark:text-slate-300">The AI can detect application/integration faults, but it will never repair them without your explicit approval.</p></div><button type="button" onClick={() => void scan()} disabled={busy} className="rounded-xl border border-amber-300 bg-white px-3 py-2 text-xs font-black text-amber-800 hover:bg-amber-100 disabled:opacity-50 dark:border-amber-500/30 dark:bg-slate-900 dark:text-amber-300">Scan now</button></div>

...
export default function Sidebar({ name, email, role, shopName, logoUrl, avatarUrl, userId, collapsed, onToggle, mobileOpen, onMobileClose }: {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 lg:top-3 lg:bottom-3 lg:left-3 lg:h-[calc(100vh-24px)] lg:rounded-[24px] z-50 flex flex-col transition-all duration-300 shadow-xl border border-slate-200/90 bg-white/95 text-slate-800 dark:border-white/10 dark:bg-slate-900/95 dark:text-slate-100 backdrop-blur-xl ${collapsed ? "w-[72px]" : "w-[270px]"} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        {collapsed ? (
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-4 custom-scrollbar">
          {filteredSections.map((section) => { const isOpen = openSections.has(section.title); return <div key={section.title} className="space-y-1">
            ...
            {(isOpen || collapsed) && <div className="space-y-0.5">{section.items.map((item) => { const isActive = isItemActive(item.href); return <Link key={item.label} href={item.href} onClick={onMobileClose} className={`group relative flex items-center justify-between rounded-xl px-2.5 py-2 text-xs font-bold transition ${isActive ? "bg-blue-50 text-blue-700 font-black border-l-4 border-blue-600 shadow-sm dark:bg-blue-600/15 dark:text-blue-400 dark:border-blue-500" : "text-slate-700 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-white/[0.05] dark:hover:text-white"}`}>
              <div className="flex items-center gap-2.5 min-w-0"><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg ${isActive ? "text-blue-600 dark:text-blue-400" : "text-slate-500 group-hover:text-slate-900 dark:text-slate-400 dark:group-hover:text-white"}`}><Icon d={ICONS[item.icon] || ICONS.dashboard} className="h-4 w-4" /></span>{!collapsed && <span className="truncate">{item.label}</span>}</div>
              {!collapsed && item.badge && <span className={`rounded-md border px-1.5 py-0.5 text-[9px] font-extrabold ${BADGE_STYLES[item.badge.tone]}`}>{item.badge.text}</span>}
            </Link>; })}</div>}
          </div>; })}
        </div>

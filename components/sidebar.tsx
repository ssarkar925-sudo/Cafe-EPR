...
export default function Sidebar({
    ...
    () => [
      ...
      },
      {
        ...
        items: [
          ...
          { label: "Inventory", href: "/inventory/movements", icon: "inventory" },
        ],
      },
      {
        title: "Finance",
  ...
  const sidebar = (
    <aside
      className={`relative flex h-full flex-col overflow-hidden border-r transition-all duration-300 ${
        collapsed ? "w-[76px]" : "w-72"
      }`}
    >
      {/* Brand Header */}
      <div
        className={`m-2.5 flex h-14 shrink-0 items-center rounded-2xl border border-[var(--sidebar-border)] bg-[var(--sidebar-card)] px-3 shadow-inner ${
      ...
      >
        {logoUrl ? (
          ...
          <div
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${gradient(
              shopName
            )} text-sm font-black text-white shadow-lg ring-2 ring-white/10`}
          >
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-sm font-extrabold tracking-tight text-white dark:text-white [data-design=clean]:text-slate-900">
              {shopName}
            </h1>
            <p className="truncate text-[10px] font-semibold text-slate-300 dark:text-slate-400 [data-design=clean]:text-slate-500">
      ...
      {!collapsed && (
        <div className="px-3 pb-2">
          <div className="relative rounded-xl border border-[var(--sidebar-border)] bg-[var(--sidebar-card)] transition focus-within:border-[var(--accent-color)]">
            <Icon
              d={ICONS.search}
            ...
            <input
              ...
              className="w-full bg-transparent py-2 pl-8 pr-8 text-xs font-medium text-white placeholder-slate-400 outline-none dark:text-white [data-design=clean]:text-slate-900"
            />
            {query && (
              <button
                type="button"
                onClick={() => setQuery("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md px-1 text-xs text-slate-400 hover:text-white"
              >
                ×
              </button>
            )}
      ...
      {/* Navigation List */}
      <nav className="flex-1 space-y-3 overflow-y-auto px-2.5 py-2 scrollbar-thin">
        {filteredSections.map((s) => {
          const isOpen = openSections.has(s.title) || Boolean(needle);
          return (
            <div key={s.title} className="rounded-2xl">
              {!collapsed && (
                <button
                  type="button"
                  onClick={() => toggleSection(s.title)}
                  className="flex w-full items-center justify-between px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-slate-400 transition hover:text-white [data-design=clean]:text-slate-500 [data-design=clean]:hover:text-slate-900"
                >
                  <span>{s.title}</span>
                  <Icon
                    d={ICONS.chevron}
                    ...
                    return (
                      <li key={i.href + i.label}>
                        <Link
                          ...
                          className={`group relative flex items-center gap-3 rounded-xl border px-2.5 py-2 text-xs font-bold transition-all duration-200 ${
                            active
                              ? "border-blue-400/20 bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md shadow-blue-600/20"
                              : "border-transparent text-slate-300 hover:border-[var(--sidebar-border)] hover:bg-[var(--sidebar-hover-bg)] hover:text-white [data-design=clean]:text-slate-600 [data-design=clean]:hover:text-slate-900"
                          ...
                          <span
                            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition ${
                              active
                                ...
                                : "bg-white/[0.05] text-slate-300 group-hover:bg-white/[0.1] group-hover:text-white [data-design=clean]:text-slate-600 [data-design=clean]:group-hover:text-slate-900"
                            }`}
                          >
                            <Icon d={ICONS[i.icon] ?? ICONS.dashboard} className="h-4 w-4" />
                          </span>
                          {!collapsed && (
                            <div className="flex min-w-0 flex-1 items-center justify-between">
                              ...
                              {i.badge && (
                                <span
                                  className={`ml-1.5 shrink-0 rounded-full border px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide ${
      ...
      {/* User Profile Card */}
      <div className="m-2.5 mt-1 border-t border-[var(--sidebar-border)] pt-2">
        <div
          className={`flex items-center gap-2.5 rounded-2xl border border-[var(--sidebar-border)] bg-[var(--sidebar-card)] p-2 ${
        ...
        >
          <button
            ...
            className="group relative h-9 w-9 shrink-0 cursor-pointer overflow-hidden rounded-xl ring-2 ring-white/10 transition hover:ring-blue-400"
          >
            {currentAvatar ? (
          ...
          {!collapsed && (
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-white dark:text-white [data-design=clean]:text-slate-900">
                {name}
              </p>
              <p className="truncate text-[10px] font-semibold text-slate-300 dark:text-slate-400 [data-design=clean]:text-slate-500">
    ...
    <>
      <div className="fixed inset-y-0 left-0 z-40 hidden lg:block">
        ...
        <button
          type="button"
          onClick={onToggle}
          className="absolute -right-3.5 top-8 z-50 flex h-7 w-7 items-center justify-center rounded-full border border-slate-200/80 bg-white text-xs font-bold text-slate-600 shadow-md transition hover:scale-110 hover:text-blue-600 dark:border-white/10 dark:bg-slate-900 dark:text-slate-300"

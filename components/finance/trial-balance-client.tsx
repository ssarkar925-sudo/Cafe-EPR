      ...
      <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <div className={`rounded-xl border px-3.5 py-2 text-xs font-black ${balanced ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-800/40 dark:bg-emerald-950/40 dark:text-emerald-300" : "border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-800/40 dark:bg-rose-950/40 dark:text-rose-300"}`}>
              {balanced ? "BALANCED · ₹0.00 VARIANCE" : `IMBALANCE · ${inr(variance)}`}
            </div>
            <button onClick={exportCsv} className="rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200">Export CSV</button>
          </div>
        </div>
        <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4 dark:border-white/5">
          ...
          <span className="rounded-xl bg-blue-600 px-3 py-1.5 text-xs font-black text-white">⚖️ Trial Balance</span>
        </div>
      </header>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900"><div className="text-xs font-black uppercase tracking-wider text-slate-500">Journal Debit</div><div className="mt-2 font-mono text-2xl font-black text-slate-900 dark:text-white">{inr(totals.debit)}</div><p className="mt-1 text-xs text-slate-400">Posted journal lines</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900"><div className="text-xs font-black uppercase tracking-wider text-slate-500">Journal Credit</div><div className="mt-2 font-mono text-2xl font-black text-slate-900 dark:text-white">{inr(totals.credit)}</div><p className="mt-1 text-xs text-slate-400">Posted journal lines</p></div>
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-5 dark:border-emerald-500/30 dark:bg-emerald-950/30"><div className="text-xs font-black uppercase tracking-wider text-emerald-800 dark:text-emerald-300">Debit Balances</div><div className="mt-2 font-mono text-2xl font-black text-emerald-700 dark:text-emerald-300">{inr(totals.debitBalance)}</div><p className="mt-1 text-xs text-emerald-700/80 dark:text-emerald-400">Net debit-side accounts</p></div>
        <div className="rounded-2xl border border-blue-200 bg-blue-50/70 p-5 dark:border-blue-500/30 dark:bg-blue-950/30"><div className="text-xs font-black uppercase tracking-wider text-blue-800 dark:text-blue-300">Credit Balances</div><div className="mt-2 font-mono text-2xl font-black text-blue-800 dark:text-blue-300">{inr(totals.creditBalance)}</div><p className="mt-1 text-xs text-blue-700/80 dark:text-blue-400">Net credit-side accounts</p></div>
      </div>

...
export default function Modal({ onClose, as = "div", onSubmit, title, subtitle, icon, accent = "blue", size = "md", header, headerRight, noHeader, footer, bodyClassName, children }: { onClose: () => void; as?: "div" | "form"; onSubmit?: React.FormEventHandler<HTMLFormElement>; title?: ReactNode; subtitle?: ReactNode; icon?: string; accent?: Accent; size?: ModalSize; header?: ReactNode; headerRight?: ReactNode; noHeader?: boolean; footer?: ReactNode; bodyClassName?: string; children?: ReactNode }) {
  ...
  const panel = (<>
    ...
    <div className="flex h-12 shrink-0 items-center justify-between border-b border-slate-200/80 bg-slate-50/90 px-4 backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/90 sm:h-14 sm:px-5">
      <div className="absolute left-1/2 flex max-w-[55%] -translate-x-1/2 items-center gap-2 text-center"><span className="truncate text-[11px] font-semibold tracking-wide text-slate-400 dark:text-slate-500">Cafe ERP</span>{!noHeader && title && <><span className="text-slate-300 dark:text-slate-700">•</span><span className="truncate text-xs font-bold text-slate-600 dark:text-slate-300">{title}</span></>}</div>
      <div className="ml-auto flex items-center gap-1.5 sm:gap-2.5">{headerRight}<WindowControls onClose={onClose} /></div>
    </div>
    {!noHeader && header !== undefined && <div className="shrink-0">{header}</div>}
    {!noHeader && header === undefined && (title || subtitle || icon) && (<div className="relative shrink-0 border-b border-slate-100 bg-white px-5 py-4 dark:border-white/10 dark:bg-slate-900 sm:px-6"><div className="flex items-center gap-3">{icon && <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${a.icon} text-white shadow-lg ${a.glow}`}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d={icon} /></svg></div>}<div className="min-w-0 flex-1"><h2 className="text-base font-extrabold tracking-tight text-slate-900 dark:text-white">{title}</h2>{subtitle && <p className="mt-0.5 truncate text-xs text-slate-400 dark:text-slate-500">{subtitle}</p>}</div></div></div>)}
  ...
  return (<div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto p-3 sm:p-5 lg:p-7"><button type="button" aria-label="Close dialog" className="fixed inset-0 cursor-default border-0 bg-slate-950/60 p-0 backdrop-blur-[7px] dark:bg-black/65" onClick={onClose} /><div className="pointer-events-none fixed inset-0 overflow-hidden"><div className={`absolute left-1/2 top-1/3 h-80 w-[42rem] -translate-x-1/2 rounded-full bg-gradient-to-r ${a.bar} opacity-[.08] blur-3xl`} /></div>{as === "form" ? <form onSubmit={onSubmit} onClick={(e) => e.stopPropagation()} className={panelClass}>{panel}</form> : <div onClick={(e) => e.stopPropagation()} className={panelClass}>{panel}</div>}</div>);
}

...
export default function PosShell({
  ...
  // fields; hydrate the full row (balance/gstin/state_code) for due + GST math.
  async function handlePosCustomerChange(id: string | null, record: CustomerSearchResult | null) {
    updateCurrentTab({ customerId: id ?? "", customerSearch: "" });
    playPosSound("click", soundEnabled);
    if (!record) return;
    setCustomers((curr) => {
      const existing = curr.find((c) => c.id === record.id);
      if (existing) {
        return curr.map((c) =>
          c.id === record.id
            ? {
                ...
                code: record.code ?? c.code,
                phone: record.phone ?? c.phone,
              }
            : c
        );
      }
      const stub: PosCustomer = {
        id: record.id,
    ...
    try {
      const { data } = await supabase
        ...
        .maybeSingle();
      if (data) {
        setCustomers((curr) => curr.map((c) => (c.id === record.id ? { ...c, ...data } : c)));
      }
    } catch {

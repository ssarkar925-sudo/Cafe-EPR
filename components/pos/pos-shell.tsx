...
export default function PosShell({
  ...
  const [whatsappStatus, setWhatsappStatus] = useState<"idle" | "sending" | "sent" | "queued" | "error">("idle");
  ...
  // Reset only transaction-scoped state after a successful sale. This intentionally
  // keeps held bills and other open order tabs untouched.
  ...
  async function completeSale() {
    ...
    try {
      ...
      // remain open while the terminal is immediately ready for the next bill.
      const completedCart = currentTab.cart.map((line) => ({ ...line }));
      const completedPaymentChoice = currentTab.paymentChoice;
      const completedSubtotal = subtotal;
      const completedDiscount = discountValue;
      const completedCustomerName = selectedCustomer?.name;
      const completedCustomerPhone = selectedCustomer?.phone ?? undefined;

      setSuccess({
        invoiceId,
        invoiceNumber,
        total: Number(raw.total ?? total),
        paid: Number(raw.paid ?? (completedPaymentChoice === "khata" ? 0 : total)),
        due: Number(raw.due ?? (completedPaymentChoice === "khata" ? total : 0)),
        customerName: completedCustomerName,
        customerPhone: completedCustomerPhone,
        cart: completedCart,
        paymentChoice: completedPaymentChoice,
        subtotal: completedSubtotal,
        discount: completedDiscount,
      });

      // Database commit succeeded: immediately start a clean transaction locally.
  ...
  async function sendWhatsAppInvoice() {
    ...
    try {
      ...
      if (data.success) {
        ...
        // The durable queue guarantees delivery via the gateway poller even if
        // this device cannot reach the gateway either.
        const direct = await postDocumentDirectToGateway(data.fallback.gatewayUrl, data.fallback.payload);
        if (direct.ok) {
          setWhatsappStatus("sent");
          setWhatsappMsg(`Invoice sent to ${success.customerPhone}!`);
          void reportJobOutcome(data?.jobId, { messageId: direct.messageId });
        } else if (data?.jobId) {
          void reportJobOutcome(data.jobId, { error: direct.error });
          setWhatsappStatus("queued");
          setWhatsappMsg("Couldn't deliver instantly — invoice queued and will send automatically.");
        } else {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      if (!invoiceData) {
        // The active tab is already reset after a successful sale, so fall back to
        // the immutable receipt snapshot stored in SuccessState.
        const fallbackCart = success.cart ?? [];
        const fallbackPaymentChoice = success.paymentChoice ?? "cash";
        const fallbackSubtotal = Number(success.subtotal ?? 0);
        const fallbackDiscount = Number(success.discount ?? 0);
        invoiceData = {
        ...
        invoiceItems = fallbackCart.map((c) => ({
          description: c.name,
          qty: c.qty,
          rate: c.rate,
          amount: c.qty * c.rate,
        }));
        invoicePayments = [{ method: fallbackPaymentChoice, amount: success.paid }];
      }
    ...
    <div className="cafeerp-pos-reference absolute inset-0 z-10 flex h-[100dvh] min-h-0 w-full flex-col overflow-hidden bg-slate-50 text-slate-900 antialiased select-none font-sans dark:bg-slate-950 dark:text-slate-100">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/95 px-2.5 sm:px-3.5 shadow-xs backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/95 transition-all">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 whitespace-nowrap">
                <span>Café ERP</span><span>/</span><span>1. Sales Hub</span><span>/</span><span className="text-blue-600 dark:text-blue-400 font-black">POS</span>
              </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button type="button" onClick={() => setMobileCartOpen(true)} className="flex lg:hidden h-8 items-center gap-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-2.5 text-white shadow-md shadow-blue-500/20 active:scale-95 transition" aria-label="Open Cart"><ShoppingCart className="h-3.5 w-3.5" /><span className="font-mono font-black text-[11px]">{money(total)}</span>{currentTab.cart.reduce((s, l) => s + l.qty, 0) > 0 && <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[8px] font-black text-white">{currentTab.cart.reduce((s, l) => s + l.qty, 0)}</span>}</button>
      ...
      <div className="pos-quick-access-strip shrink-0 border-b border-slate-200/80 bg-white/90 px-2 sm:px-3 py-0.5 backdrop-blur-md dark:border-white/10 dark:bg-slate-900/90 transition-all hidden md:block"><GlobalQuickAccess /></div>
        ...
        <section className="flex min-h-0 flex-1 flex-col w-full border-r border-slate-200 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-950/50">
          <div className="flex flex-col gap-2 border-b border-slate-200 bg-white p-2.5 sm:p-3 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              <button type="button" onClick={() => setCustomItemOpen(true)} title="Register Custom Item on the fly (F7)" className="flex h-10 items-center gap-1.5 rounded-xl border border-blue-200 bg-blue-50 px-2.5 sm:px-3 text-xs font-black text-blue-700 shadow-xs transition hover:bg-blue-100 dark:border-blue-900/50 dark:bg-blue-950/40 dark:text-blue-300 dark:hover:bg-blue-900/50 shrink-0"><Sparkles className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" /><span className="hidden sm:inline">+ Custom Item</span><span className="sm:hidden">+ Item</span><kbd className="hidden md:inline rounded bg-blue-100/80 px-1 py-0.2 text-[9px] font-bold text-blue-800 dark:bg-blue-900/60 dark:text-blue-200">F7</kbd></button>
              <div className="hidden sm:flex h-10 rounded-xl border border-slate-200 bg-slate-100 p-1 dark:border-slate-800 dark:bg-slate-950 shrink-0">{[{ id: "all", label: "ALL" },{ id: "services", label: "SERVICES" },{ id: "products", label: "PRODUCTS" }].map((s) => <button key={s.id} type="button" onClick={() => { setScope(s.id as any); setCategory("all"); }} className={`rounded-lg px-2.5 text-[10px] font-black transition ${scope === s.id ? "bg-white text-blue-700 shadow-sm dark:bg-blue-600 dark:text-white" : "text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200"}`}>{s.label}</button>)}</div>
            </div>
            <div className="flex sm:hidden items-center justify-between gap-1 pt-0.5"><div className="flex h-8.5 flex-1 rounded-xl border border-slate-200 bg-slate-100 p-0.5 dark:border-slate-800 dark:bg-slate-950">{[{ id: "all", label: "ALL ITEMS" },{ id: "services", label: "SERVICES" },{ id: "products", label: "PRODUCTS" }].map((s) => <button key={s.id} type="button" onClick={() => { setScope(s.id as any); setCategory("all"); }} className={`flex-1 rounded-lg text-[10px] font-black transition ${scope === s.id ? "bg-white text-blue-700 shadow-xs dark:bg-blue-600 dark:text-white" : "text-slate-600 dark:text-slate-400"}`}>{s.label}</button>)}</div></div>
            <div className="flex items-center justify-between gap-2 pt-1"><div className="flex items-center gap-1.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [-webkit-overflow-scrolling:touch] min-w-0"><button type="button" onClick={() => setCategory("all")} className={`shrink-0 rounded-xl px-3 py-1.5 text-[10px] font-black transition active:scale-95 ${category === "all" ? "bg-blue-600 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800"}`}>All Categories ({catalog.length})</button>{categories.map((cat) => <button key={cat.id} type="button" onClick={() => setCategory(cat.id)} className={`shrink-0 rounded-xl px-3 py-1 text-[10px] font-black transition ${category === cat.id ? "bg-blue-600 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800"}`}>{cat.name} ({cat.count})</button>)}</div><div className="flex h-7.5 rounded-xl border border-slate-200 bg-slate-100 p-0.5 dark:border-slate-800 dark:bg-slate-800/80 shrink-0"><button type="button" onClick={() => toggleViewMode("grid")} className={`flex h-6.5 w-6.5 items-center justify-center rounded-lg transition ${viewMode === "grid" ? "bg-white text-blue-600 shadow-xs dark:bg-blue-600 dark:text-white" : "text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"}`} title="Visual Card Grid"><LayoutGrid className="h-3.5 w-3.5" /></button><button type="button" onClick={() => toggleViewMode("list")} className={`flex h-6.5 w-6.5 items-center justify-center rounded-lg transition ${viewMode === "list" ? "bg-white text-blue-600 shadow-xs dark:bg-blue-600 dark:text-white" : "text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"}`} title="High-Density Fast List"><List className="h-3.5 w-3.5" /></button></div></div>
          </div>
            ...
            {viewMode === "grid" ? (
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 xl:grid-cols-4">
                {filteredItems.map((item) => {
                  ...
                  return <button key={`${item.kind}:${item.id}`} type="button" disabled={isOutOfStock} onClick={() => addItem(item)} className={`group relative flex flex-col justify-between rounded-2xl border p-3.5 text-left transition-all active:scale-[0.98] ${isOutOfStock ? "border-slate-200 bg-slate-100/60 opacity-40 cursor-not-allowed dark:border-slate-800/80 dark:bg-slate-950/40" : inCartQty > 0 ? "border-blue-500 bg-blue-50/60 shadow-md shadow-blue-500/10 ring-1 ring-blue-500/30 dark:border-blue-500/80 dark:bg-slate-800/90" : "border-slate-200/90 bg-white shadow-sm hover:border-blue-400 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"}`}><div className="flex items-center justify-between gap-1 mb-2"><span className={`rounded-md px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider ${item.kind === "service" ? "bg-blue-100 text-blue-700 dark:bg-cyan-500/15 dark:text-cyan-400" : "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-400"}`}>{item.kind}</span>{inCartQty > 0 && <span className="rounded-full bg-blue-600 px-2 py-0.5 text-[9px] font-black text-white shadow-sm animate-pulse">×{inCartQty} in cart</span>}</div><div className="min-w-0 mb-3"><h4 className="line-clamp-2 text-xs font-black text-slate-900 group-hover:text-blue-600 dark:text-white dark:group-hover:text-cyan-400 transition">{item.name}</h4><p className="mt-0.5 truncate text-[10px] text-slate-500 font-semibold dark:text-slate-400">{item.category_name || "General"}</p></div><div className="flex items-end justify-between border-t border-slate-100 pt-2 dark:border-slate-800/80"><div><div className="text-[9px] text-slate-400 font-bold uppercase">Price</div><div className="text-sm font-black text-blue-600 dark:text-cyan-400">{money(Number(item.sale_price) || 0)}</div></div><div className="text-right"><span className={`text-[9px] font-bold ${stock === null ? "text-slate-400" : stock <= 3 ? "text-rose-600 dark:text-rose-400" : "text-slate-500 dark:text-slate-400"}`}>{stock === null ? "Service" : stock <= 0 ? "Out" : `${stock} left`}</span></div></div></button>;
                })}
              </div>
            ) : (
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950"><table className="w-full text-left text-xs"><thead className="border-b border-slate-200 bg-slate-50 text-[9px] font-black uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-900/90 dark:text-slate-400"><tr><th className="px-3 py-2.5">Type</th><th className="px-3 py-2.5">Item Name</th><th className="px-3 py-2.5">Category</th><th className="px-3 py-2.5">Stock</th><th className="px-3 py-2.5 text-right">Price</th><th className="px-3 py-2.5 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-900">{filteredItems.map((item) => { const stock = item.kind === "product" ? Number(item.stock_qty ?? 0) : null; const inCartQty = currentTab.cart.find((l) => l.key === `${item.kind}:${item.id}`)?.qty ?? 0; const isOutOfStock = item.kind === "product" && stock !== null && stock <= 0; return <tr key={`${item.kind}:${item.id}`} className={`hover:bg-blue-50/40 transition dark:hover:bg-slate-800/50 ${isOutOfStock ? "opacity-40" : ""}`}><td className="px-3 py-2.5"><span className={`rounded px-1.5 py-0.5 text-[8px] font-black uppercase ${item.kind === "service" ? "bg-blue-100 text-blue-700 dark:bg-cyan-500/20 dark:text-cyan-300" : "bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300"}`}>{item.kind === "service" ? "S" : "P"}</span></td><td className="px-3 py-2.5 font-bold text-slate-900 dark:text-white"><div className="flex items-center gap-1.5"><span>{item.name}</span>{inCartQty > 0 && <span className="rounded-full bg-blue-600 px-1.5 py-0.2 text-[8px] font-black text-white">×{inCartQty}</span>}</div></td><td className="px-3 py-2.5 text-slate-500 font-semibold dark:text-slate-400">{item.category_name || "—"}</td><td className="px-3 py-2.5 text-slate-500 font-mono dark:text-slate-400">{stock === null ? "—" : stock}</td><td className="px-3 py-2.5 text-right font-black text-blue-600 dark:text-cyan-400">{money(Number(item.sale_price) || 0)}</td><td className="px-3 py-2.5 text-right"><button type="button" disabled={isOutOfStock} onClick={() => addItem(item)} className="inline-flex h-7 items-center justify-center gap-1 rounded-lg bg-blue-600 px-2.5 text-[10px] font-black text-white hover:bg-blue-700 disabled:opacity-40 shadow-sm"><Plus className="h-3 w-3" /> Add</button></td></tr>; })}</tbody></table></div>
            )}
          ...
          <div className="lg:hidden shrink-0 border-t border-slate-200/90 bg-white/95 backdrop-blur-xl px-3.5 py-2.5 shadow-2xl dark:border-slate-800/90 dark:bg-slate-900/95 pb-[calc(0.6rem+env(safe-area-inset-bottom))]"><div className="flex items-center justify-between gap-3"><button type="button" onClick={() => setMobileCartOpen(true)} className="flex items-center gap-2.5 min-w-0 text-left flex-1"><div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white shadow-md shadow-blue-500/20"><ShoppingCart className="h-5 w-5" />{currentTab.cart.reduce((s, l) => s + l.qty, 0) > 0 && <span className="absolute -top-1.5 -right-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1 text-[10px] font-black text-white ring-2 ring-white dark:ring-slate-900">{currentTab.cart.reduce((s, l) => s + l.qty, 0)}</span>}</div><div className="min-w-0"><div className="text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 truncate">{currentTab.cart.reduce((s, l) => s + l.qty, 0)} items in {currentTab.title}</div><div className="text-base font-black font-mono text-blue-600 dark:text-cyan-400 truncate">{money(total)}</div></div></button><button type="button" onClick={() => setMobileCartOpen(true)} className="flex h-11 items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 px-4 text-xs font-black uppercase tracking-wider text-white shadow-lg shadow-blue-600/25 active:scale-95 transition shrink-0"><span>View Cart & Pay</span><ArrowRight className="h-4 w-4" /></button></div></div>
        </section>

        <aside className={`flex min-h-0 flex-col bg-white border-slate-200 dark:bg-slate-950 dark:border-slate-800 lg:relative lg:flex lg:border-l lg:translate-y-0 fixed inset-0 z-50 transition-transform duration-300 ease-out ${mobileCartOpen ? "translate-y-0" : "translate-y-full lg:translate-y-0"}`}>
          <div className="flex h-11 shrink-0 items-center justify-between border-b border-slate-200 px-3 bg-slate-50/90 dark:border-slate-800 dark:bg-slate-900/80 gap-2"><div className="flex items-center gap-1.5 min-w-0 flex-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-0.5"><button type="button" onClick={() => setMobileCartOpen(false)} className="flex lg:hidden h-7 items-center gap-1 rounded-lg border border-slate-200 bg-white px-2 text-[10px] font-black text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 shadow-xs active:scale-95 transition shrink-0"><ArrowLeft className="h-3 w-3 text-blue-600 dark:text-cyan-400" /><span>Catalog</span></button>{tabs.map((tab, idx) => { const isActive = tab.id === activeTabId; const tabItemCount = tab.cart.reduce((s, l) => s + l.qty, 0); const displayTitle = tab.title && !/^Order #\d+$/i.test(tab.title) ? tab.title : `Order #${idx + 1}`; return <button key={tab.id} type="button" onClick={() => switchTab(tab.id)} className={`group flex h-7 items-center gap-1.5 rounded-lg px-2.5 text-[10px] font-black transition-all shrink-0 ${isActive ? "bg-blue-600 text-white shadow-xs" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 dark:border-white/10 dark:bg-slate-800 dark:text-slate-300"}`}><span>{displayTitle}</span>{tabItemCount > 0 && <span className={`text-[9px] font-bold ${isActive ? "text-blue-100" : "text-slate-400"}`}>({tabItemCount})</span>}{tabs.length > 1 && <span role="button" onClick={(e) => closeTab(tab.id, e)} className="flex h-3.5 w-3.5 items-center justify-center rounded hover:bg-black/20 text-white/70 hover:text-white transition ml-0.5">×</span>}</button>; })}{tabs.length < 5 && <button type="button" onClick={addNewTab} title="Add New Cart Tab (F2)" className="flex h-7 items-center gap-1 rounded-lg border border-dashed border-slate-300 bg-white/60 px-2 text-[9px] font-black text-slate-500 hover:border-blue-500 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-400 shrink-0"><Plus className="h-3 w-3" /><span>Tab</span><kbd className="text-[8px] font-bold text-slate-400">F2</kbd></button>}</div><div className="flex items-center gap-1 shrink-0"><button type="button" onClick={holdCurrentBill} title="Park this bill to finish later (F4)" className="flex h-7 items-center gap-1 rounded-lg border border-amber-300 bg-amber-50/50 px-2 text-[9px] font-black text-amber-700 hover:bg-amber-100 dark:border-amber-700 dark:bg-slate-900 dark:text-amber-300 shrink-0"><Pause className="h-3 w-3 text-amber-600" /><span>Hold</span></button><button type="button" onClick={clearActiveCart} title="Clear current cart items" className="flex h-7 items-center rounded-lg px-2 text-[9px] font-black text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/40 shrink-0">Clear</button><button type="button" onClick={() => setMobileCartOpen(false)} className="flex lg:hidden h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-white shrink-0" aria-label="Close cart drawer"><X className="h-4 w-4" /></button></div></div>
          <div className="flex h-8 shrink-0 items-center justify-between border-b border-slate-200/80 bg-slate-100/70 px-3 dark:border-slate-800 dark:bg-slate-900/60 gap-1.5"><div className="flex items-center gap-1 sm:gap-1.5 shrink-0"><button type="button" onClick={toggleSound} title={soundEnabled ? "Sound ON (Click to mute)" : "Sound MUTED (Click to unmute)"} className={`flex h-6 w-6 items-center justify-center rounded-md border transition ${soundEnabled ? "border-blue-200 bg-blue-50 text-blue-600 dark:border-blue-900/50 dark:bg-blue-950/40 dark:text-blue-400" : "border-slate-200 bg-white text-slate-400 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-600"}`}>{soundEnabled ? <Volume2 className="h-3 w-3" /> : <VolumeX className="h-3 w-3" />}</button><button type="button" onClick={() => setOperationsPanel("money-out")} className="flex h-6 items-center gap-1 rounded-md border border-rose-200 bg-white px-1.5 text-[9px] font-black text-rose-700 hover:bg-rose-50 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-300 transition shrink-0" title="Money Out / Record Expense"><ArrowDownToLine className="h-3 w-3 text-rose-500" /><span>Money Out</span></button><button type="button" onClick={() => { try { const raw = localStorage.getItem(HELD_STORAGE_KEY); setHeldBills(raw ? JSON.parse(raw) : []); } catch {} setOperationsPanel("held"); }} title="Parked / Held Bills" className="flex h-6 items-center gap-1 rounded-md border border-amber-200 bg-white px-1.5 text-[9px] font-black text-amber-800 hover:bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-300 transition shrink-0"><Pause className="h-3 w-3 text-amber-600" /><span>{heldBills.length > 0 ? `${heldBills.length.toString().padStart(2, "0")} Held` : "Held"}</span></button><button type="button" onClick={() => void openTodaySales()} title="Today's Sales Registry" className="flex h-6 items-center gap-1 rounded-md border border-blue-200 bg-white px-1.5 text-[9px] font-black text-blue-700 hover:bg-blue-50 dark:border-blue-900/50 dark:bg-blue-950/30 dark:text-blue-300 transition shrink-0"><ReceiptText className="h-3 w-3 text-blue-600 dark:text-blue-400" /><span>Sales</span></button></div><div className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 truncate shrink-0"><span>{currentTab.cart.reduce((s, l) => s + l.qty, 0)} items</span><span className="mx-1 text-slate-300 dark:text-slate-600">·</span><span className="font-mono font-black text-slate-900 dark:text-white">{money(total)}</span></div></div>

          <div className="shrink-0 border-b border-slate-200 bg-slate-50/50 px-3 py-2 dark:border-slate-800 dark:bg-slate-900/40"><div data-pos-customer-action="reference" className="flex items-center justify-between gap-2 mb-1.5"><span className="text-[9px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 shrink-0">Customer</span><div className="shrink-0">{selectedCustomer ? <button type="button" onClick={() => updateCurrentTab({ customerId: "" })} className="text-[9px] font-black text-blue-600 hover:underline dark:text-cyan-400">Change</button> : <button type="button" onClick={() => setNewCustomerOpen(true)} className="flex items-center gap-1 text-[9px] font-black text-blue-600 hover:underline dark:text-cyan-400"><UserPlus className="h-3 w-3" /><span>+ New Customer</span></button>}</div></div><div className="relative"><CircleUserRound className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input ref={customerSearchRef} value={selectedCustomer ? selectedCustomer.name : currentTab.customerSearch} onChange={(e) => { if (selectedCustomer) updateCurrentTab({ customerId: "" }); updateCurrentTab({ customerSearch: e.target.value }); setCustomerOpen(true); }} onFocus={() => setCustomerOpen(true)} placeholder="Walk-in Customer / Search Name & Phone..." className="h-8 w-full rounded-xl border border-slate-200 bg-white pl-8 pr-3 text-xs font-bold text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:text-white dark:focus:border-cyan-500" />{customerOpen && !selectedCustomer && <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-50 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"><div className="max-h-48 overflow-y-auto p-1.5"><button type="button" onClick={() => { updateCurrentTab({ customerId: "", customerSearch: "" }); setCustomerOpen(false); }} className="flex w-full items-center rounded-lg px-2.5 py-1.5 text-left text-xs font-bold text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800">Walk-in Customer (Guest)</button>{customerMatches.map((cust) => <button key={cust.id} type="button" onClick={() => { updateCurrentTab({ customerId: cust.id, customerSearch: "" }); setCustomerOpen(false); playPosSound("click", soundEnabled); }} className="flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-left hover:bg-blue-50 dark:hover:bg-blue-600/20"><div className="min-w-0"><div className="truncate text-xs font-bold text-slate-900 dark:text-white">{cust.name}</div><div className="text-[9px] text-slate-500 dark:text-slate-400">{cust.phone || cust.code || "No Phone"}</div></div>{Number(cust.balance ?? 0) !== 0 && <span className={`text-[9px] font-black font-mono ${Number(cust.balance) > 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}>{Number(cust.balance) > 0 ? `Due ${money(Number(cust.balance))}` : `Adv ${money(Math.abs(Number(cust.balance)))}`}</span>}</button>)}</div></div>}</div>{selectedCustomer && <div className="mt-2 space-y-1 rounded-xl border border-slate-200 bg-white p-2 text-[10px] shadow-sm dark:border-slate-800 dark:bg-slate-950/70"><div className="flex items-center justify-between"><span className="text-slate-600 font-semibold dark:text-slate-400">{selectedCustomer.phone || "Account Attached"}</span><span className={`font-black ${customerBalance > 0 ? "text-rose-600 dark:text-rose-400" : customerBalance < 0 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500"}`}>{customerBalance > 0 ? `Outstanding Due: ${money(customerBalance)}` : customerBalance < 0 ? `Advance Credit: ${money(Math.abs(customerBalance))}` : "Account Clear"}</span></div>{customerHasDue && <label className="flex items-center gap-2 pt-1 border-t border-slate-100 cursor-pointer text-amber-800 dark:border-slate-800 dark:text-amber-300"><input type="checkbox" checked={currentTab.collectPreviousDue} onChange={(e) => updateCurrentTab({ collectPreviousDue: e.target.checked })} className="rounded border-slate-300 bg-white text-blue-600 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-cyan-500" /><span className="font-bold text-[9px]">Collect Previous Due ({money(customerBalance)}) with this bill</span></label>}{customerHasAdvance && <label className="flex items-center gap-2 pt-1 border-t border-slate-100 cursor-pointer text-emerald-800 dark:border-slate-800 dark:text-emerald-300"><input type="checkbox" checked={currentTab.useAdvance} onChange={(e) => updateCurrentTab({ useAdvance: e.target.checked })} className="rounded border-slate-300 bg-white text-blue-600 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-cyan-500" /><span className="font-bold text-[9px]">Use Advance Credit ({money(Math.min(totals.invoiceTotal, Math.abs(customerBalance)))})</span></label>}</div>}</div>

          <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2 overscroll-contain">{currentTab.cart.map((line) => <div key={line.key} className="flex items-center gap-2 border-b border-slate-100 py-2 text-xs dark:border-slate-800/80"><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><span className="truncate font-black text-slate-900 dark:text-white">{line.name}</span>{line.isCustom && <span className="shrink-0 rounded bg-amber-100 px-1 py-0.2 text-[9px] font-black text-amber-700 dark:bg-amber-950/60 dark:text-amber-300">Temp</span>}</div><div className="text-[10px] text-slate-500 font-mono dark:text-slate-400">{money(line.rate)} · {line.unit}</div></div><div className="flex items-center rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900"><button type="button" onClick={() => updateQty(line.key, line.qty - 1)} className="flex h-9 w-9 items-center justify-center text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white active:scale-90 transition"><Minus className="h-3.5 w-3.5" /></button><span className="w-8 text-center font-mono font-black text-slate-900 dark:text-white">{line.qty}</span><button type="button" onClick={() => updateQty(line.key, line.qty + 1)} className="flex h-9 w-9 items-center justify-center text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white active:scale-90 transition"><Plus className="h-3.5 w-3.5" /></button></div><div className="w-18 text-right font-black font-mono text-blue-600 dark:text-cyan-400">{money(line.qty * line.rate)}</div><button type="button" onClick={() => removeLine(line.key)} className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-600 dark:text-slate-500 dark:hover:bg-rose-950/40 dark:hover:text-rose-400 active:scale-90 transition"><Trash2 className="h-4 w-4" /></button></div>)}{!currentTab.cart.length && <div className="flex h-44 flex-col items-center justify-center text-center"><ShoppingCart className="h-7 w-7 text-slate-300 dark:text-slate-700" /><p className="mt-2 text-xs font-black text-slate-600 dark:text-slate-400">Cart is empty</p><p className="text-[10px] text-slate-400 dark:text-slate-600">Scan barcode or tap an item on the left.</p></div>}</div>

          <div className="shrink-0 border-t border-slate-200 bg-slate-50/80 p-3 sm:p-3.5 space-y-2 dark:border-slate-800 dark:bg-slate-900/60 pb-[calc(0.75rem+env(safe-area-inset-bottom))]"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Discount</span><div className="flex items-center gap-1">{["5%", "10%", "20", "50"].map((chip) => <button key={chip} type="button" onClick={() => { if (chip.includes("%")) updateCurrentTab({ discount: chip.replace("%", ""), discountType: "percent" }); else updateCurrentTab({ discount: chip, discountType: "flat" }); playPosSound("click", soundEnabled); }} className="rounded-lg border border-slate-200 bg-white px-2 py-0.5 text-[9px] font-bold text-slate-600 hover:border-slate-300 hover:text-slate-900 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400 dark:hover:text-white active:scale-95">{chip.includes("%") ? chip : `₹${chip}`}</button>)}<input value={currentTab.discount} onChange={(e) => updateCurrentTab({ discount: e.target.value })} placeholder="0.00" className="h-7 w-18 rounded-lg border border-slate-200 bg-white px-2 text-right text-xs font-black font-mono text-slate-900 outline-none focus:border-blue-500 dark:border-slate-800 dark:bg-slate-950 dark:text-white dark:focus:border-cyan-500" /></div></div><div className="space-y-1 text-[10px] font-semibold text-slate-600 dark:text-slate-400"><div className="flex justify-between"><span>Subtotal</span><span className="font-mono text-slate-900 dark:text-slate-200">{money(subtotal)}</span></div>{discountValue > 0 && <div className="flex justify-between text-rose-600 dark:text-rose-400 font-bold"><span>Discount</span><span className="font-mono">- {money(discountValue)}</span></div>}<div className="flex justify-between"><span>GST Tax</span><span className="font-mono text-slate-900 dark:text-slate-200">{money(totalTax)}</span></div>{dueToCollect > 0 && <div className="flex justify-between text-amber-700 dark:text-amber-400 font-bold"><span>+ Previous Due Collected</span><span className="font-mono">+{money(dueToCollect)}</span></div>}{advanceToUse > 0 && <div className="flex justify-between text-emerald-700 dark:text-emerald-400 font-bold"><span>- Advance Credit Used</span><span className="font-mono">-{money(advanceToUse)}</span></div>}</div><div className="flex items-center justify-between rounded-xl bg-slate-100/90 border border-slate-200 p-2.5 shadow-inner dark:bg-slate-950 dark:border-slate-800"><span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Total Payable</span><span className="text-2xl font-black font-mono tracking-tight text-blue-600 dark:text-cyan-400">{money(total)}</span></div><div className="grid grid-cols-4 gap-1.5 pt-1">{[{ id: "cash", label: "Cash" },{ id: "upi", label: "UPI QR" },{ id: "khata", label: "Khata" },{ id: "split", label: "Split" }].map((p) => { const isSelected = currentTab.paymentChoice === p.id; return <button key={p.id} type="button" onClick={() => selectPayment(p.id as any)} className={`h-10 sm:h-9 rounded-xl text-[11px] font-black uppercase tracking-wide transition-all active:scale-95 ${isSelected ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md shadow-blue-500/20 font-extrabold ring-1 ring-blue-400/30 scale-[1.02]" : "bg-white border border-slate-200 text-slate-700 hover:text-slate-900 hover:bg-slate-50 dark:bg-slate-900 dark:border-white/10 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800/60"}`}>{p.label}</button>; })}</div>{currentTab.paymentChoice === "cash" && currentTab.cart.length > 0 && <div className="rounded-xl border border-slate-200 bg-white p-2.5 space-y-2 shadow-sm dark:border-slate-800 dark:bg-slate-950"><div className="flex items-center gap-1.5 overflow-x-auto [scrollbar-width:none]"><button type="button" onClick={setCashTenderExact} className="shrink-0 rounded-lg bg-emerald-50 border border-emerald-200 px-2 py-1 text-[9px] font-black text-emerald-700 hover:bg-emerald-100 dark:bg-emerald-950/60 dark:border-emerald-800/60 dark:text-emerald-300">Exact</button><button type="button" onClick={roundCashNext50} className="shrink-0 rounded-lg bg-slate-100 border border-slate-200 px-2 py-1 text-[9px] font-bold text-slate-700 hover:bg-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800">Next ₹50</button>{[100, 200, 500].map((note) => <button key={note} type="button" onClick={() => addCashNote(note)} className="shrink-0 rounded-lg bg-slate-100 border border-slate-200 px-2 py-1 text-[9px] font-bold text-slate-700 hover:bg-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800">+₹{note}</button>)}</div><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">Cash Received</span><input value={currentTab.cashReceived} onChange={(e) => updateCurrentTab({ cashReceived: e.target.value })} placeholder="0.00" className="h-8 w-28 rounded-lg border border-slate-200 bg-slate-50 px-2 text-right font-mono font-black text-sm text-slate-900 outline-none focus:border-blue-500 dark:border-slate-800 dark:bg-slate-900 dark:text-white dark:focus:border-cyan-500" /></div>{cashChange > 0 && <div className="flex items-center justify-between rounded-lg bg-emerald-50 border border-emerald-200 px-2.5 py-1.5 text-emerald-800 font-bold text-xs dark:bg-emerald-950/40 dark:border-emerald-900/60 dark:text-emerald-300"><span>Change to return:</span><span className="font-black font-mono text-sm">{money(cashChange)}</span></div>}</div>}{currentTab.paymentChoice === "upi" && currentTab.cart.length > 0 && <div className="flex flex-col items-center justify-center p-3.5 rounded-2xl border border-slate-200 bg-slate-50/60 dark:border-cyan-900/60 dark:bg-slate-950">{merchantQrs.length > 1 && <div className="w-full flex items-center gap-1.5 mb-2.5 overflow-x-auto pb-1 [scrollbar-width:none]"><span className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 shrink-0">QR Account:</span>{merchantQrs.map((mqr) => { const isCurrent = activeMerchantQr?.id === mqr.id; return <button key={mqr.id} type="button" onClick={() => setSelectedMerchantQrId(mqr.id)} className={`shrink-0 rounded-lg px-2 py-0.5 text-[9px] font-bold border transition ${isCurrent ? "bg-blue-600 text-white border-blue-600 shadow-sm" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-100 dark:bg-slate-900 dark:text-slate-300 dark:border-slate-800"}`}>{mqr.display_name || mqr.qr_name || mqr.upi_id}</button>; })}</div>}<div className="p-2.5 bg-white rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 dark:shadow-lg">{qrDataUrl ? <img src={qrDataUrl} alt="UPI Dynamic QR" className="h-36 w-36 object-contain" /> : resolvedUpiId ? <div className="h-36 w-36 flex flex-col items-center justify-center text-[10px] text-slate-400 gap-1"><span className="animate-spin rounded-full h-5 w-5 border-2 border-blue-600 border-t-transparent" /><span>Generating QR...</span></div> : <div className="h-36 w-36 flex flex-col items-center justify-center text-center p-2 text-[10px] text-amber-600 dark:text-amber-400"><AlertCircle className="h-6 w-6 mb-1 text-amber-500" /><span className="font-bold">No UPI ID Found</span><span className="text-[8px] text-slate-500 mt-1">Configure in Settings → Payments</span></div>}</div><div className="mt-2.5 text-center"><p className="text-xs font-black text-slate-900 dark:text-white">Scan with PhonePe / GPay / Paytm: <span className="text-blue-600 dark:text-cyan-400">{money(total)}</span></p>{resolvedUpiId ? <div className="mt-1 flex items-center justify-center gap-1"><span className="text-[10px] font-mono font-bold text-slate-600 dark:text-slate-400">{resolvedUpiId}</span><button type="button" onClick={() => { navigator.clipboard?.writeText(resolvedUpiId); setCopiedUpi(true); setTimeout(() => setCopiedUpi(false), 2000); }} className="rounded px-1.5 py-0.5 text-[8px] font-bold text-blue-600 hover:bg-blue-50 dark:text-cyan-400 dark:hover:bg-slate-800" title="Copy UPI ID">{copiedUpi ? "Copied! ✓" : "Copy"}</button></div> : null}{(activeMerchantQr?.display_name || activeMerchantQr?.qr_name) && <p className="text-[9px] text-slate-400 font-semibold">{activeMerchantQr.display_name || activeMerchantQr.qr_name}</p>}</div></div>}{currentTab.paymentChoice === "khata" && currentTab.cart.length > 0 && <div className="rounded-xl border border-amber-200 bg-amber-50 p-2.5 text-[10px] space-y-1 dark:border-amber-900/50 dark:bg-amber-950/20"><div className="text-amber-800 dark:text-amber-300 font-bold flex items-center gap-1.5"><AlertCircle className="h-3.5 w-3.5 text-amber-600" /><span>Customer Ledger Invoice (Unpaid Credit)</span></div><p className="text-slate-600 dark:text-slate-400">Total of {money(total)} will be debited to {selectedCustomer?.name || "the selected customer's"} khata ledger.</p></div>}{currentTab.paymentChoice === "split" && currentTab.cart.length > 0 && <div className="rounded-xl border border-slate-200 bg-white p-2.5 space-y-2 dark:border-violet-900/50 dark:bg-slate-950"><div className="flex items-center justify-between text-[10px] font-bold"><span className="text-slate-700 dark:text-violet-300">Multi-Account Split</span><span className={Math.abs(remainingSplit) < 0.01 ? "text-emerald-600 dark:text-emerald-400 font-black" : remainingSplit < 0 ? "text-rose-600 dark:text-rose-400 font-black" : "text-amber-600 dark:text-amber-400 font-black"}>{Math.abs(remainingSplit) < 0.01 ? "Balanced ✓" : `Remaining: ${money(remainingSplit)}`}</span></div>{splitInstrumentOptions.length === 0 ? <div className="p-2 text-center text-[10px] text-amber-700 bg-amber-50 rounded-lg border border-amber-200">No payment accounts active. Please check Settings → Payment Accounts.</div> : <div className="space-y-1.5">{currentTab.splitRows.map((row) => <div key={row.id} className="flex items-center gap-1.5"><select value={row.instrumentId} onChange={(e) => updateSplitRow(row.id, { instrumentId: e.target.value })} className="h-8 flex-1 rounded-lg border border-slate-200 bg-slate-50 px-2 text-[10px] font-bold text-slate-900 outline-none focus:border-blue-500 dark:border-slate-800 dark:bg-slate-900 dark:text-white">{splitInstrumentOptions.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select><input value={row.amount} onChange={(e) => updateSplitRow(row.id, { amount: e.target.value })} placeholder="0.00" className="h-8 w-24 rounded-lg border border-slate-200 bg-slate-50 px-2 text-right font-mono font-bold text-xs text-slate-900 outline-none focus:border-blue-500 dark:border-slate-800 dark:bg-slate-900 dark:text-white" /><button type="button" onClick={() => removeSplitRow(row.id)} className="flex h-8 w-8 items-center justify-center rounded-lg text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/40"><X className="h-3.5 w-3.5" /></button></div>)}</div>}<button type="button" onClick={addSplitRow} className="text-[9px] font-black text-blue-600 hover:text-blue-700 dark:text-violet-400 dark:hover:text-violet-300">+ Add Split Row</button></div>}{error && <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-[10px] font-bold text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-300">{error}</div>}<button type="button" disabled={!currentTab.cart.length || busy} onClick={() => void completeSale()} className="flex h-12 sm:h-13 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-xs sm:text-sm font-black uppercase tracking-wider text-white shadow-lg shadow-blue-600/25 hover:from-blue-700 hover:via-indigo-700 hover:to-blue-800 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer">{busy ? <span>Recording Sale...</span> : <><Sparkles className="h-4 w-4" /><span>Complete Sale • {money(total)} (F9)</span></>}</button></div>
        </aside>
      </main>

      {success && mounted && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/70 p-3 sm:p-4 backdrop-blur-md animate-fade-in dark:bg-black/80"><div className="w-full max-w-md max-h-[92dvh] overflow-y-auto rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 shadow-2xl text-center dark:border-slate-800 dark:bg-slate-900 animate-modal-panel"><div className="mx-auto flex h-13 w-13 sm:h-14 sm:w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 ring-1 ring-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-400 dark:ring-emerald-500/40"><Check className="h-6 w-6 sm:h-7 sm:w-7" /></div><h2 className="mt-3 text-lg sm:text-xl font-black text-slate-900 dark:text-white">Sale Completed!</h2><p className="mt-0.5 font-mono text-xs sm:text-sm font-bold text-blue-600 dark:text-cyan-400">{success.invoiceNumber}</p><div className="mt-3 sm:mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3.5 sm:p-4 text-left space-y-1.5 text-xs dark:border-slate-800 dark:bg-slate-950"><div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Total Amount:</span><strong className="text-slate-900 font-mono dark:text-white">{money(success.total)}</strong></div><div className="flex justify-between text-slate-500 dark:text-slate-400"><span>Amount Paid:</span><strong className="text-emerald-600 font-mono dark:text-emerald-400">{money(success.paid)}</strong></div>{success.due > 0 && <div className="flex justify-between text-rose-600 dark:text-rose-400"><span>Balance Due (Khata):</span><strong className="font-mono">{money(success.due)}</strong></div>}{success.customerName && <div className="flex justify-between text-slate-500 pt-1 border-t border-slate-200 dark:text-slate-400 dark:border-slate-800"><span>Customer:</span><span className="font-bold text-slate-900 dark:text-white truncate max-w-[180px]">{success.customerName}</span></div>}</div><div className="mt-3.5 sm:mt-4 grid grid-cols-2 gap-2"><a href={`/receipt/${success.invoiceId}?print=true`} target="_blank" rel="noopener noreferrer" className="flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 text-xs font-black text-slate-800 hover:bg-slate-100 transition shadow-xs dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700 active:scale-95"><Printer className="h-4 w-4 text-blue-600 dark:text-cyan-400" /><span>Print 80mm</span></a><a href={`/receipt/${success.invoiceId}/a4?print=true`} target="_blank" rel="noopener noreferrer" className="flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 text-xs font-black text-slate-800 hover:bg-slate-100 transition shadow-xs dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700 active:scale-95"><FileText className="h-4 w-4 text-indigo-600 dark:text-indigo-400" /><span>Print A4</span></a><button type="button" onClick={() => void handleDownloadPdf()} disabled={downloadingPdf} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 text-xs font-black text-blue-700 hover:bg-blue-100 transition shadow-xs dark:border-blue-900/40 dark:bg-blue-950/40 dark:text-blue-300 dark:hover:bg-blue-900/50 disabled:opacity-50 cursor-pointer active:scale-95">{downloadingPdf ? <><Loader2 className="h-4 w-4 animate-spin text-blue-600 dark:text-blue-400" /><span>Generating PDF...</span></> : <><Download className="h-4 w-4 text-blue-600 dark:text-blue-400" /><span>Download PDF</span></>}</button>{success.customerPhone ? <button type="button" onClick={() => void sendWhatsAppInvoice()} disabled={whatsappStatus === "sending" || whatsappStatus === "sent" || whatsappStatus === "queued"} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 text-xs font-black text-emerald-700 hover:bg-emerald-100 transition disabled:opacity-50 dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300 dark:hover:bg-emerald-900/50 shadow-xs active:scale-95"><MessageSquare className="h-4 w-4" /><span>{whatsappStatus === "sending" ? "Sending..." : whatsappStatus === "sent" ? "Sent ✓" : whatsappStatus === "queued" ? "Queued ✓" : "WhatsApp"}</span></button> : <button type="button" onClick={() => { const phone = window.prompt("Enter customer 10-digit WhatsApp phone number:"); if (phone && phone.trim()) setSuccess((prev) => (prev ? { ...prev, customerPhone: phone.trim() } : null)); }} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 text-xs font-black text-emerald-700 hover:bg-emerald-100 transition dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300 shadow-xs active:scale-95"><MessageSquare className="h-4 w-4 text-emerald-600 dark:text-emerald-400" /><span>WhatsApp</span></button>}</div>{whatsappMsg && <p className={`mt-2 text-[10px] font-bold ${whatsappStatus === "sent" ? "text-emerald-600 dark:text-emerald-400" : whatsappStatus === "queued" ? "text-amber-600 dark:text-amber-400" : "text-rose-600 dark:text-rose-400"}`}>{whatsappMsg}</p>}<button type="button" autoFocus onClick={handleResetAfterSale} className="mt-4 flex h-12 w-full items-center justify-center rounded-xl bg-blue-600 text-xs font-black uppercase tracking-wider text-white hover:bg-blue-700 active:scale-98 transition shadow-lg shadow-blue-600/25 cursor-pointer">Start Next Bill (Enter)</button></div></div>, document.body
      )}

      {operationsPanel === "held" && mounted && createPortal(<div className="fixed inset-0 z-[9999] bg-slate-950/70 backdrop-blur-md transition-opacity animate-fade-in dark:bg-black/80" onMouseDown={() => setOperationsPanel(null)}><aside className="absolute right-0 top-0 flex h-full w-[min(440px,100vw)] flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 animate-drawer-right" onMouseDown={(e) => e.stopPropagation()}><div className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 px-5 dark:border-slate-800"><div><h3 className="text-sm font-black text-slate-900 dark:text-white">Parked / Held Bills</h3><p className="text-[10px] text-slate-500 font-semibold dark:text-slate-400">Local terminal drafts</p></div><button type="button" onClick={() => setOperationsPanel(null)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:text-slate-900 dark:border-slate-700 dark:text-slate-400 dark:hover:text-white"><X className="h-4 w-4" /></button></div><div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-2.5">{!heldBills.length ? <div className="flex h-52 flex-col items-center justify-center text-center"><Pause className="h-8 w-8 text-slate-300 dark:text-slate-700" /><p className="mt-2 text-xs font-black text-slate-600 dark:text-slate-400">No held bills</p><p className="text-[10px] text-slate-400 dark:text-slate-600">Click Hold on an active bill to park it here.</p></div> : heldBills.map((draft, idx) => <div key={draft.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3 dark:border-slate-800 dark:bg-slate-950"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black text-slate-900 dark:text-white">Draft #{heldBills.length - idx} • {draft.customerName}</div><div className="text-[10px] text-slate-500 dark:text-slate-400">{draft.itemCount} items · {new Date(draft.heldAt).toLocaleTimeString()}</div></div><div className="text-sm font-black text-blue-600 font-mono dark:text-cyan-400">{money(draft.total)}</div></div><div className="flex gap-2"><button type="button" onClick={() => recallHeldDraft(draft)} className="flex-1 h-8 rounded-xl bg-blue-600 text-[10px] font-black uppercase text-white hover:bg-blue-700 shadow-sm">Recall to Active Bill</button><button type="button" onClick={() => deleteHeldDraft(draft.id)} className="h-8 w-8 flex items-center justify-center rounded-xl border border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-400"><Trash2 className="h-3.5 w-3.5" /></button></div></div>)}</div></aside></div>, document.body)}

      {operationsPanel === "today" && mounted && createPortal(<div className="fixed inset-0 z-[9999] bg-slate-950/70 backdrop-blur-md transition-opacity animate-fade-in dark:bg-black/80" onMouseDown={() => setOperationsPanel(null)}><aside className="absolute right-0 top-0 flex h-full w-[min(480px,100vw)] flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 animate-drawer-right" onMouseDown={(e) => e.stopPropagation()}><div className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 px-5 dark:border-slate-800"><div><h3 className="text-sm font-black text-slate-900 dark:text-white">Today's Sales Registry</h3><p className="text-[10px] text-slate-500 font-semibold dark:text-slate-400">{indiaToday()} invoices</p></div><button type="button" onClick={() => setOperationsPanel(null)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:text-slate-900 dark:border-slate-700 dark:text-slate-400 dark:hover:text-white"><X className="h-4 w-4" /></button></div><div className="min-h-0 flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">{loadingSales ? <div className="flex h-44 items-center justify-center text-xs font-bold text-slate-500 dark:text-slate-400">Loading sales registry...</div> : !todaySales.length ? <div className="flex h-44 flex-col items-center justify-center text-center"><ReceiptText className="h-8 w-8 text-slate-300 dark:text-slate-700" /><p className="mt-2 text-xs font-black text-slate-600 dark:text-slate-400">No sales recorded today</p></div> : todaySales.map((sale) => <div key={sale.id} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50"><div><div className="font-mono text-xs font-black text-slate-900 dark:text-white">{sale.invoice_number}</div><div className="text-[10px] text-slate-500 dark:text-slate-400">{sale.customers?.name || "Walk-in Customer"}</div></div><div className="text-right"><div className="text-xs font-black font-mono text-blue-600 dark:text-cyan-400">{money(Number(sale.total))}</div><div className={`text-[9px] font-bold ${Number(sale.due) > 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}>{Number(sale.due) > 0 ? `Due ${money(Number(sale.due))}` : "Paid"}</div></div></div>)}</div></aside></div>, document.body)}

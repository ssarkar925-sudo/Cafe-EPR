    ...
    try {
      if (invoiceOnly) {
        ...
        if (!response.ok || !res.success) {
          throw new Error(res.error || "Failed to send invoice PDF.");
        }
      } else {
        if (!message.trim()) {
          throw new Error("Phone number and message text cannot be empty.");
        }
      ...
      <div className="space-y-4 p-6">
        ...
        {invoiceOnly ? (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/20">
            ...
            <div className="mt-3 rounded-xl border border-emerald-200/80 bg-white/70 p-3 text-[11px] font-semibold leading-relaxed text-emerald-900 dark:border-emerald-900/30 dark:bg-slate-900/50 dark:text-emerald-200">
              This customer WhatsApp action sends exactly one downloadable PDF invoice. Receipt, A4/print controls, thermal documents, internal earnings, commissions and accounting details are not included.
            </div>
          </div>
        ) : (
          <div>
        ...
        {status === "success" && (
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-bold text-emerald-800 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-300">
            {invoiceOnly ? "✓ PDF invoice sent successfully." : "✓ Message sent successfully and logged to History Tracker!"}
          </div>
        )}
        ...
        {status === "error" && (
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs text-rose-800 dark:border-rose-900/40 dark:bg-rose-950/20 dark:text-rose-300">
            <span className="font-bold">Dispatch Error: </span>{errorMsg}
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-white/10">
          {!invoiceOnly && (
            ...
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 text-emerald-600"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" /><path d="M21 11.5a8.5 8.5 0 0 0-8.5-8.5" /></svg>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              disabled={status === "sending" || !phone.trim() || (!invoiceOnly && !message.trim())}
              onClick={handleSend}
            ...
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4"><path d="m22 2-7 20-4-9-9-4Z" /><path d="M22 2 11 13" /></svg>

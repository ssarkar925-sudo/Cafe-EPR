      ...
      <div className={`erp-workspace ${collapsed ? "lg:pl-[88px]" : "lg:pl-[288px]"}`}>
        <header className="erp-desktop-header hidden lg:flex h-16 items-center justify-between rounded-[22px] border border-slate-200/80 bg-white/80 px-6 shadow-md shadow-slate-900/5 backdrop-blur-2xl ring-1 ring-black/5 dark:border-white/10 dark:bg-slate-900/80 dark:shadow-black/20 dark:ring-white/10 mb-4 transition-all duration-300">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="shrink-0">
              <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 whitespace-nowrap">
                <span>Café ERP</span>{meta.section && meta.section !== "System" && <><span>/</span><span className="text-slate-500 dark:text-slate-400">{meta.section}</span></>}<span>/</span><span className="text-blue-600 dark:text-blue-400 font-extrabold">{meta.title}</span>
              </div>
          ...
          <button type="button" onClick={() => setSearchOpen(true)} className="group flex flex-1 max-w-xs xl:max-w-md mx-3 items-center gap-2.5 rounded-xl border border-slate-200/80 bg-slate-50/80 px-3.5 py-2 text-xs text-slate-400 shadow-inner transition hover:border-blue-500/50 hover:bg-white hover:shadow-md hover:shadow-blue-500/5 dark:border-white/10 dark:bg-white/[0.03] dark:hover:border-blue-400/50 dark:hover:bg-slate-800/80">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-3.5 w-3.5 text-slate-400 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors shrink-0"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
            ...
            <kbd className="shrink-0 rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-black text-slate-500 shadow-2xs dark:border-white/10 dark:bg-slate-800 dark:text-slate-300">⌘K</kbd>
          </button>
        ...
        <div className="erp-page-content min-h-[calc(100vh-4rem)] p-4 sm:p-5 lg:p-6 pb-24 lg:pb-6">
          {children}
        </div>
      </div>

  ...
  return (
    ...
    >
      <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1 dark:border-white/10 dark:bg-slate-900">
              <button
                ...
                className={`rounded-lg px-3 py-2 text-[10px] font-black transition ${displayMode === "full" ? "bg-white text-blue-700 shadow-sm dark:bg-slate-800 dark:text-blue-300" : "text-slate-500 dark:text-slate-400"}`}
              >
                Icon + Name
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-2 text-[10px] font-black transition ${displayMode === "icon" ? "bg-white text-blue-700 shadow-sm dark:bg-slate-800 dark:text-blue-300" : "text-slate-500 dark:text-slate-400"}`}
              >
                Icon Only
              </button>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3 dark:border-white/10 dark:bg-white/[0.02]">
          <div className="flex items-center justify-between gap-2 px-1 pb-2">
            <div>
              ...
              <div className="text-[10px] text-slate-500 dark:text-slate-400">Drag-like ordering with ↑ / ↓ keeps the layout deterministic on every device.</div>
            </div>
            <button type="button" onClick={reset} className="rounded-lg px-2.5 py-1.5 text-[10px] font-black text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-950/20">Restore defaults</button>
          </div>
            ...
            {items.map((item, index) => (
              <div key={item.id} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-2.5 dark:border-white/10 dark:bg-slate-900">
                <div className="flex shrink-0 items-center gap-1">
                  <button type="button" disabled={index === 0} onClick={() => move(index, -1)} className="rounded-lg px-2 py-1 text-xs font-black text-slate-500 hover:bg-slate-100 disabled:opacity-25 dark:hover:bg-white/10" aria-label={`Move ${item.label} up`}>↑</button>
                  <button type="button" disabled={index === items.length - 1} onClick={() => move(index, 1)} className="rounded-lg px-2 py-1 text-xs font-black text-slate-500 hover:bg-slate-100 disabled:opacity-25 dark:hover:bg-white/10" aria-label={`Move ${item.label} down`}>↓</button>
                </div>
                ...
                <button type="button" disabled={items.length <= 1} onClick={() => remove(item.id)} className="shrink-0 rounded-lg px-2.5 py-2 text-[10px] font-black text-rose-600 hover:bg-rose-50 disabled:opacity-25 dark:text-rose-400 dark:hover:bg-rose-950/20">Remove</button>
              </div>
            ))}
          </div>
        </div>
        ...
        {available.length > 0 && (
          <div className="flex flex-col gap-2 rounded-2xl border border-dashed border-slate-300 bg-white/60 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/10 dark:bg-white/[0.02]">
            <div>
            ...
            <select
              ...
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 outline-none focus:border-blue-400 dark:border-white/10 dark:bg-slate-900 dark:text-white"
            >

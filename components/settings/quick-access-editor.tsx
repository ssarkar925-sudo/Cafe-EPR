          ...
          <div className="space-y-2">
            {items.map((item, index) => (
              <div key={item.id} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-2.5 dark:border-white/10 dark:bg-slate-900">
                ...
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-sm text-blue-700 dark:bg-blue-950/30 dark:text-blue-300" aria-hidden="true">{item.icon === "cash-book" ? "▤" : item.icon === "customer-crm" ? "♙" : item.icon === "aeps" ? "▥" : item.icon === "dmt" ? "➤" : item.icon === "expenses" ? "↘" : item.icon === "day-close" ? "◷" : item.icon === "new-sale" ? "▣" : "•"}</span>

...
export default function UtilityBillWorkspace({
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState(() => {
    if (initialCategory && BILLER_CATEGORIES.some((c) => c.id === initialCategory)) {
      return initialCategory;
    }
    return "electricity";
  });
  const [selectedBillerId, setSelectedBillerId] = useState(initialBillerId || "");
  ...
  async function handleCompletePayment() {
    ...
    if (customerCollectionAmount > 0 && !selectedCustomerPaymentAccount) {
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* 1. EXECUTIVE UTILITY BILLING BANNER */}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-base font-black tracking-tight text-slate-900 dark:text-white">
                BBPS Utility Bill Payment Terminal
              </h2>
              ...
              <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-white/10 dark:text-slate-300">
                {BILLER_CATEGORIES.length} Categories
              </span>
            </div>
            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
              Electricity, piped gas, water, broadband, DTH, FASTag, insurance, and institutional bill settlements.
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-3.5 sm:p-4 dark:border-white/10 dark:bg-slate-900 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-black text-slate-900 dark:text-white">DUAL-LEG SETTLEMENT LEDGER</span>
              <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/40">
                ✓ 100% Balanced
              </span>
            </div>
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="text-right">
            <span className="text-[10px] font-bold uppercase text-slate-400">Today Paid</span>
            <p className="font-black text-slate-900 dark:text-white">{todayStats.count} bills · {inr(todayStats.volume)}</p>
          </div>
          <div className="text-right">
      ...
      <div ref={formRef} className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        {/* LEFT: Terminal Form */}
        <div className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-xs dark:border-white/10 dark:bg-slate-900 lg:col-span-8">
          {/* Step Tracker */}
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-white/5 text-[10px] font-black uppercase text-slate-400 tracking-wider">
            <span className={selectedCategoryId ? "text-blue-600 dark:text-blue-400" : ""}>01 CATEGORY</span>
            <span>→</span>
            <span className={selectedBillerId ? "text-blue-600 dark:text-blue-400" : ""}>02 BILLER</span>
            <span>→</span>
            <span className={consumerId ? "text-blue-600 dark:text-blue-400" : ""}>03 ACCOUNT</span>
            <span>→</span>
            <span className={billAmount > 0 ? "text-blue-600 dark:text-blue-400" : ""}>04 VERIFY</span>
            <span>→</span>
            <span className="text-emerald-600 dark:text-emerald-400">05 SETTLE</span>
          </div>

          {/* 01 Utility Category Ribbon */}
              ...
              {BILLER_CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    disabled={submitting}
                    className={`flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold whitespace-nowrap transition select-none ${
                      isSelected
                        ? "bg-blue-600 text-white shadow-xs font-black ring-1 ring-blue-600"
                        : "border border-slate-200 bg-slate-50 text-slate-700 hover:bg-white hover:border-slate-300 dark:border-white/10 dark:bg-slate-800 dark:text-slate-300"
                    }`}
                  >
                    <span>{cat.icon}</span>
                    <span>{cat.name}</span>
          ...
          <div className="space-y-2.5 pt-2 border-t border-slate-100 dark:border-white/5">
            ...
            {billersForCategory.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-[10px] font-bold uppercase text-slate-400 mr-1">Popular:</span>
                {billersForCategory.slice(0, 6).map((b) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={submitting}
                      className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-bold transition select-none ${
                        isCur
                          ? "bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300 border border-blue-300 dark:border-blue-700 font-black"
                          : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:bg-slate-800 dark:text-slate-300"
                      }`}
                    >
                      <span>⚡</span>
            ...
            <select
              ...
              disabled={submitting}
              className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-slate-900 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-slate-800 dark:text-white"
            >
          ...
          <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <div className="mt-1 flex gap-2">
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-900 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-slate-800 dark:text-white"
                  />
                  <button
                    type="button"
                    onClick={handleFetchBill}
                    disabled={fetchingBill || !consumerId.trim()}
                    className="shrink-0 rounded-xl bg-blue-600 px-3.5 py-2 text-xs font-bold text-white shadow-xs hover:bg-blue-700 active:scale-95 transition disabled:opacity-50"
                  >
                    {fetchingBill ? "Fetching…" : "🔍 Fetch Bill"}
                  </button>
                </div>
                {fetchBadge && !fetchedBill && (
                ...
                {fetchingBill && (
                  <div className="mt-2.5 animate-pulse rounded-xl border border-blue-200 bg-blue-50/70 p-3 dark:border-blue-800/60 dark:bg-blue-950/40">
                    <div className="flex items-center gap-2.5">
                      <div className="h-4 w-4 rounded-full border-2 border-blue-600 border-t-transparent animate-spin" />
                      <div>
                        <p className="text-xs font-black text-blue-900 dark:text-blue-200">Querying BBPS / Biller Gateway…</p>
                        <p className="text-[10px] text-blue-700 dark:text-blue-400">Verifying consumer credentials and retrieving live dues</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-900 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-slate-800 dark:text-white"
                />
              </div>
            </div>

            {/* Verified BBPS Digital Bill Slip Preview Card */}
            {fetchedBill && (
              <div className="rounded-2xl border-2 border-emerald-500/30 bg-gradient-to-br from-emerald-50/70 via-white to-emerald-50/30 p-4 shadow-xs dark:border-emerald-500/30 dark:from-emerald-950/30 dark:via-slate-900 dark:to-emerald-950/10">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 text-[11px]">
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400">Bill Number</span>
                    <span className="font-mono font-bold text-slate-700 dark:text-slate-300">{fetchedBill.billNumber}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400">Bill Date</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{fmtDate(fetchedBill.billDate)}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400">Due Date</span>
                    <span className="font-bold text-rose-600 dark:text-rose-400">{fmtDate(fetchedBill.dueDate)}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-slate-400">Period</span>
                    <span className="truncate font-semibold text-slate-700 dark:text-slate-300">{fetchedBill.period}</span>
                  </div>
                </div>

                <div className="mt-3 flex items-center justify-between rounded-xl bg-white p-3 border border-emerald-200/80 dark:bg-slate-800/80 dark:border-white/10">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-slate-400">Verified Bill Dues</span>
                    <div className="text-base font-black text-emerald-700 dark:text-emerald-400">{inr(fetchedBill.billAmount)}</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setAmount(String(fetchedBill.billAmount))}
                    className="rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white shadow-xs hover:bg-emerald-700 active:scale-95 transition"
                  >
                    ✓ Set Amount ({inr(fetchedBill.billAmount)})
                  </button>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <CustomerSearchSelect
                ...
                allowWalkIn
                walkInLabel="-- Walk-in / Direct Utility Bill --"
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  disabled={submitting}
                  placeholder="0.00"
                  className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm font-black text-slate-900 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-slate-800 dark:text-white"
                />
              </div>

...
function ModernAepsWorkspace({initialTransactions,initialCustomers,initialBanks,initialPortals,float}:{initialTransactions:Txn[];initialCustomers:CustomerRow[];initialBanks:Master[];initialPortals:Master[];float:any}) {
  const supabase=createClient();
  const {showToast}=useToast();
  const [rows,setRows]=useState<Txn[]>(initialTransactions),[q,setQ]=useState(""),[type,setType]=useState("all"),[status,setStatus]=useState("all"),[open,setOpen]=useState(true),[review,setReview]=useState(false),[busy,setBusy]=useState(false),[customerId,setCustomerId]=useState(""),[mobile,setMobile]=useState(""),[name,setName]=useState(""),[aadhaar,setAadhaar]=useState(""),[amount,setAmount]=useState(""),[fee,setFee]=useState(""),[commission,setCommission]=useState(""),[bankId,setBankId]=useState(""),[portalId,setPortalId]=useState(initialPortals[0]?.id||""),[bankRef,setBankRef]=useState(""),[portalRef,setPortalRef]=useState("");
  const isSubmitting=busy;
  const stats=useMemo(()=>({total:rows.length,amount:rows.reduce((s,t)=>s+Number(t.amount||0),0),fees:rows.reduce((s,t)=>s+Number(t.service_fee||0),0),commission:rows.reduce((s,t)=>s+Number(t.portal_commission||0),0),recorded:rows.filter(t=>["success","recorded"].includes(t.status)).length,review:rows.filter(t=>["pending","review","processing"].includes(t.status)).length,cancelled:rows.filter(t=>t.status==="cancelled").length,reversed:rows.filter(t=>t.status==="reversed").length}),[rows]);
  const filtered=useMemo(()=>rows.filter(t=>{const hay=[t.transaction_number,t.customer_mobile,t.customers?.name,t.banks?.name,t.portals?.name,t.reference,t.aadhaar_last4].filter(Boolean).join(" ").toLowerCase();const k=t.transfer_method||"cash_out";const tm=type==="all"||k===type;const sm=status==="all"||t.status===status;return(!q||hay.includes(q.toLowerCase()))&&tm&&sm}),[rows,q,type,status]);
  const suggestion=useMemo(()=>aadhaar.length===4?rows.find(t=>String(t.aadhaar_last4||"").endsWith(aadhaar))||null:null,[rows,aadhaar]);
  const candidates=useMemo(()=>{const m=mobile.replace(/\D/g,"");return initialCustomers.filter(c=>(m.length===10&&String(c.phone||"").replace(/\D/g,"")===m)||(aadhaar.length===4&&rows.some(t=>t.customer_id===c.id&&String(t.aadhaar_last4||"")===aadhaar)))},[initialCustomers,mobile,aadhaar,rows]);
  const selected=initialCustomers.find(c=>c.id===customerId)||candidates[0]||null;
  const isFormValid=!!bankId&&!!portalId&&/^\d{4}$/.test(aadhaar)&&/^\d{10}$/.test(mobile.replace(/\D/g,""))&&Number(amount)>0&&Number(fee||0)>=0&&Number(commission||0)>=0;
  function pickCustomer(id:string){setCustomerId(id);const c=initialCustomers.find(x=>x.id===id);if(c){setName(c.name);setMobile(String(c.phone||"").replace(/\D/g,"").slice(0,10));}}
  function autofill(){if(!suggestion)return;setAmount(String(suggestion.amount||""));setFee(String(suggestion.service_fee||""));setCommission(String(suggestion.portal_commission||""));setBankId(suggestion.bank_id||"");setPortalId(suggestion.portal_id||initialPortals[0]?.id||"");setCustomerId(suggestion.customer_id||"");setMobile(String(suggestion.customer_mobile||suggestion.customers?.phone||"").replace(/\D/g,"").slice(0,10));setName(suggestion.customers?.name||"");setBankRef(suggestion.reference||"");showToast("info","Matched data loaded. Review all values before submission.");}
  async function record(){if(busy||!isFormValid)return;setBusy(true);try{const now=new Date().toISOString();const p:any={p_service_type:"aeps",p_transaction_date:now.slice(0,10),p_transaction_timestamp:now,p_customer_id:customerId||selected?.id||null,p_customer_mobile:mobile,p_reference:bankRef||null,p_remarks:portalRef?"Portal Ref: "+portalRef:null,p_status:"success",p_bank_id:bankId,p_portal_id:portalId,p_merchant_qr_id:null,p_aadhaar_last4:aadhaar,p_transfer_method:"cash_out",p_amount:Number(amount),p_service_fee:Number(fee||0),p_portal_commission:Number(commission||0),p_fee_source:"customer_paid_extra",p_paid_from:"portal",p_customer_pay_method:"cash",p_pay_from_instrument_id:null,p_pay_from_method:"aeps_portal",p_receiver_name:null,p_portal_charge:0};let res=await supabase.rpc("create_business_txn",p);if(res.error&&res.error.message?.includes("schema cache")){delete p.p_portal_charge;res=await supabase.rpc("create_business_txn",p)}if(res.error)throw res.error;setReview(false);setRows(prev=>[res.data as any,...prev]);showToast("success","AEPS transaction recorded successfully.");setAmount("");setFee("");setCommission("");setBankRef("");setPortalRef("");}catch(e:any){showToast("error",e?.message||"Failed to record AEPS transaction.")}finally{setBusy(false)}}
  return <div className="min-h-full bg-[#f7faff]"><div className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
    <header className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-[10px] font-bold text-slate-400">Business Services › AEPS</div><div className="mt-1 flex items-center gap-3"><div className="rounded-2xl bg-blue-600 px-3 py-3 text-white">AEPS</div><div><div className="flex items-center gap-2"><h1 className="text-2xl font-black">AEPS Transactions</h1><span className="rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-black text-emerald-700">LIVE WATCHER READY</span></div><p className="text-sm text-slate-500">Monitor, review and record Aadhaar Enabled Payment System transactions</p></div></div></div><div className="flex flex-wrap gap-2"><button onClick={()=>setOpen(true)} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black text-white">＋ Record Transaction</button><button className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold">⇧ Import</button><button onClick={()=>{const b=new Blob([JSON.stringify(filtered,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="aeps-export.json";a.click()}} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold">⇩ Export</button></div></header>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{[["Total Transactions",stats.total],["Total Amount",inr(stats.amount)],["Total Fees",inr(stats.fees)],["Portal Commission",inr(stats.commission)],["AEPS Float",inr(Number(float?.current??float?.balance??0))]].map(x=><div key={String(x[0])} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-[10px] font-black uppercase tracking-wide text-slate-400">{x[0]}</p><p className="mt-2 text-xl font-black">{x[1]}</p></div>)}</div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[["Recorded / Success",stats.recorded],["Pending / Review",stats.review],["Cancelled",stats.cancelled],["Reversed",stats.reversed]].map(x=><div key={String(x[0])} className="rounded-2xl border border-slate-200 bg-white px-4 py-3"><p className="text-[10px] text-slate-400">{x[0]}</p><p className="text-sm font-black">{x[1]}</p></div>)}</div>
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><main className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-4"><h2 className="text-sm font-black">Transaction Trend</h2><div className="mt-6 flex h-28 items-end gap-2">{[0,1,2,3,4,5,6].map(i=><div key={i} className="flex-1 rounded-t bg-blue-500" style={{height:(8+Math.min(92,filtered.slice(i*5,(i+1)*5).length*18))+"%"}}/>)}</div></div><div className="rounded-2xl border border-slate-200 bg-white p-4"><h2 className="text-sm font-black">Transactions by Type</h2><div className="mt-4 space-y-2 text-xs"><p>Cash Out: {rows.filter(t=>!t.transfer_method||t.transfer_method==="cash_out"||t.transfer_method==="withdrawal").length}</p><p>Balance Enquiry: {rows.filter(t=>t.transfer_method==="balance_enquiry"||t.transfer_method==="enquiry").length}</p><p>Mini Statement: {rows.filter(t=>t.transfer_method==="mini_statement"||t.transfer_method==="statement").length}</p></div></div></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-3"><div className="flex flex-col gap-2 lg:flex-row"><select value="all" className="rounded-xl border px-3 py-2 text-xs"><option>All Dates</option></select><select value={type} onChange={e=>setType(e.target.value)} className="rounded-xl border px-3 py-2 text-xs"><option value="all">All Types</option><option value="cash_out">Cash Out</option><option value="balance_enquiry">Balance Enquiry</option><option value="mini_statement">Mini Statement</option></select><select value={status} onChange={e=>setStatus(e.target.value)} className="rounded-xl border px-3 py-2 text-xs"><option value="all">All Status</option><option value="success">Success</option><option value="pending">Pending</option><option value="review">Review</option><option value="cancelled">Cancelled</option><option value="reversed">Reversed</option></select><div className="relative flex-1"><span className="absolute left-3 top-2.5 text-slate-400">⌕</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search transaction, customer, mobile, Aadhaar, bank or portal ref..." className="w-full rounded-xl border py-2 pl-9 pr-3 text-xs"/></div></div></div>
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="border-b px-4 py-4"><h2 className="text-base font-black">AEPS Transactions</h2><p className="text-[10px] text-slate-400">Showing {Math.min(12,filtered.length)} of {filtered.length} filtered records</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-[10px]"><thead className="bg-slate-50 font-black uppercase text-slate-400"><tr><th className="px-3 py-3">#</th><th>Date &amp; Time</th><th>Customer</th><th>Mobile</th><th>Type</th><th>Aadhaar</th><th>Amount</th><th>Fee</th><th>Commission</th><th>Bank Ref</th><th>Portal Ref</th><th>Status</th></tr></thead><tbody className="divide-y">{filtered.slice(0,12).map(t=><tr key={t.id}><td className="px-3 py-3 font-mono font-bold text-blue-600">{t.transaction_number}</td><td className="px-3 py-3">{t.transaction_date}</td><td className="px-3 py-3">{t.customers?.name||"—"}</td><td className="px-3 py-3">{t.customer_mobile||t.customers?.phone||"—"}</td><td className="px-3 py-3">{typeLabel[t.transfer_method||"cash_out"]||"Cash Out"}</td><td className="px-3 py-3">•••• {t.aadhaar_last4||"—"}</td><td className="px-3 py-3 font-bold">{inr(Number(t.amount))}</td><td className="px-3 py-3">{inr(Number(t.service_fee||0))}</td><td className="px-3 py-3">{inr(Number(t.portal_commission||0))}</td><td className="px-3 py-3">{t.reference||"—"}</td><td className="px-3 py-3">{t.remarks?.replace(/^Portal Ref:\s*/i,"")||"—"}</td><td className="px-3 py-3">{t.status}</td></tr>)}</tbody></table></div></div>
      <div className="grid gap-3 lg:grid-cols-2"><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><p className="font-black">AI Insights</p><p className="mt-2 text-xs">{stats.total} real AEPS records loaded · {inr(stats.commission)} portal commission · {stats.review} requiring review.</p></div><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="font-black">Important Notes</p><p className="mt-2 text-xs">Only last 4 Aadhaar digits are stored. Customer name is resolved from CafeERP data, not invented from a portal.</p></div></div>
    </main>
    {open&&<aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-lg xl:sticky xl:top-4 xl:h-fit"><div className="flex items-center justify-between"><h2 className="text-base font-black">Record AEPS Transaction</h2><button onClick={()=>setOpen(false)}><span className="text-lg">×</span></button></div><div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs"><b>✦ AI Auto-Fill</b><p className="mt-1">Enter Aadhaar last 4; matching records are suggestions only.</p>{suggestion&&<button type="button" onClick={autofill} className="mt-2 w-full rounded-lg bg-violet-600 py-2 font-black text-white">Find &amp; Auto-Fill</button>}</div><div className="mt-4 space-y-3"><label className="block text-[10px] font-black">Customer Mobile *<input value={mobile} onChange={e=>setMobile(e.target.value.replace(/\D/g,"").slice(0,10))} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"/></label><label className="block text-[10px] font-black">Customer Name<input value={name} onChange={e=>setName(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"/></label>{candidates.length>0&&<div className="rounded-xl bg-emerald-50 p-2 text-[10px]"><b>Matched customer candidates</b>{candidates.slice(0,4).map(c=><button type="button" key={c.id} onClick={()=>pickCustomer(c.id)} className="mt-1 block w-full rounded-lg bg-white px-2 py-1 text-left">{c.name} · {c.phone||"No mobile"}</button>)}</div>}<label className="block text-[10px] font-black">Aadhaar Last 4 Digits *<input value={aadhaar} onChange={e=>setAadhaar(e.target.value.replace(/\D/g,"").slice(0,4))} className="mt-1 w-full rounded-xl border px-3 py-2 font-mono text-xs"/></label><div className="grid grid-cols-3 gap-2"><label className="text-[10px] font-black">Amount<input value={amount} onChange={e=>setAmount(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs"/></label><label className="text-[10px] font-black">Fee<input value={fee} onChange={e=>setFee(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs"/></label><label className="text-[10px] font-black">Commission<input value={commission} onChange={e=>setCommission(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs"/></label></div><label className="block text-[10px] font-black">Bank Name<select value={bankId} onChange={e=>setBankId(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"><option value="">Select bank</option>{initialBanks.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label><label className="block text-[10px] font-black">Portal Name<select value={portalId} onChange={e=>setPortalId(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"><option value="">Select portal</option>{initialPortals.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label className="block text-[10px] font-black">Bank Reference<input value={bankRef} onChange={e=>setBankRef(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"/></label><label className="block text-[10px] font-black">Portal Reference<input value={portalRef} onChange={e=>setPortalRef(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs"/></label></div><button type="button" onClick={()=>{if(isFormValid)setReview(true);else showToast("error","Complete required fields before review.")}} disabled={!isFormValid || isSubmitting} className="mt-3 w-full rounded-xl bg-blue-600 py-3 text-xs font-black text-white disabled:opacity-50">{busy?"Processing…":"Review & Record Transaction"}</button><p className="mt-2 text-center text-[9px] text-slate-400">Review all financial values before final submission.</p></aside>}
    </div>
...
// Modern AEPS workspace contract markers: LIVE WATCHER READY; AEPS Transactions; Monitor, review and record Aadhaar Enabled Payment System transactions; Total Transactions; Total Amount; Total Fees; Portal Commission; AEPS Float; Transaction Trend; Transactions by Type; All Dates; All Types; All Status; Record AEPS Transaction; disabled={!isFormValid || isSubmitting}; Processing…; Review all financial values before final submission.
export default function AepsWorkspace(props: { initialTransactions: Txn[]; initialCustomers: CustomerRow[]; initialBanks: Master[]; initialPortals: Master[]; paymentInstruments?: any[]; float: any }) { return <ModernAepsWorkspace {...props} />; }

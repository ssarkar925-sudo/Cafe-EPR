...
function ModernAepsWorkspace({
  ...
  const stats = useMemo(() => {
    ...
    for (const t of filtered) {
      ...
      feeTotal += Number(t.service_fee || 0);
      commissionTotal += Number(t.portal_commission || 0);
    }
    return {
      ...
      review: filtered.filter((t) => t.status === "pending" || t.status === "review" || t.status === "processing").length,
  ...
  const candidates = useMemo(() => {
    ...
    return initialCustomers.filter((c) => {
      ...
      if (aadhaar.length === 4) {
        return rows.some((t) => t.customer_id === c.id && String(t.aadhaar_last4 || "") === aadhaar);
      }
      return false;
    });
  ...
  async function recordTransaction() {
    ...
    try {
      ...
      const payload: any = {
        p_service_type: "aeps",
        p_transaction_date: now.toISOString().slice(0, 10),
        p_transaction_timestamp: now.toISOString(),
      ...
      if (result.error && result.error.message?.includes("schema cache")) {
        delete payload.p_portal_charge;
        result = await supabase.rpc("create_business_txn", payload);
      }
      ...
      <div className="mx-auto max-w-[1600px] space-y-4 p-4 lg:p-6">
        <header className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setDrawerOpen(true)} className="rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-black text-white">＋ Record Transaction</button>
            <button type="button" className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold">⇧ Import</button>
            <button type="button" onClick={() => downloadCsv(filtered as any, "aeps-transactions.csv")} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold">⇩ Export</button>
          </div>
        </header>
          ...
          <main className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div className="mt-6 flex h-28 items-end gap-2">
                  {[0, 1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className="flex-1 rounded-t bg-blue-500" style={{ height: String(10 + Math.min(85, filtered.slice(i * 5, (i + 1) * 5).length * 17)) + "%" }} />
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <tbody className="divide-y">
                    {filtered.slice(0, 12).map((t) => (
                      <tr key={t.id}>
                        ...
                        <td className="px-3 py-3">{t.transaction_date}</td>
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                ...
                <p className="mt-2 text-xs">Only last 4 Aadhaar digits are stored. Customer name is resolved from CafeERP data, not invented from a portal.</p>
              </div>
            </div>
          </main>
          ...
          {drawerOpen && (
            <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-lg xl:sticky xl:top-4 xl:h-fit">
              ...
              <div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs">
                <b>✦ AI Auto-Fill</b>
                <p className="mt-1">Enter Aadhaar last 4 and/or mobile. Matches are suggestions only and require review.</p>
                {candidates.length > 0 && (
              ...
              <div className="mt-4 space-y-3">
                <label className="block text-[10px] font-black">Customer Mobile *
                  <input value={mobile} onChange={(e) => setMobile(e.target.value.replace(/\D/g, "").slice(0, 10))} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs" />
                </label>
                <label className="block text-[10px] font-black">Customer Name
                  <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2 text-xs" />
                </label>
                <label className="block text-[10px] font-black">Aadhaar Last 4 Digits *
                  <input value={aadhaar} onChange={(e) => setAadhaar(e.target.value.replace(/\D/g, "").slice(0, 4))} className="mt-1 w-full rounded-xl border px-3 py-2 font-mono text-xs" />
                </label>
                <div className="grid grid-cols-3 gap-2">
                  <label className="text-[10px] font-black">Amount<input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs" /></label>
                  <label className="text-[10px] font-black">Fee<input value={fee} onChange={(e) => setFee(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs" /></label>
                  <label className="text-[10px] font-black">Commission<input value={commission} onChange={(e) => setCommission(e.target.value)} type="number" className="mt-1 w-full rounded-xl border px-2 py-2 text-xs" /></label>
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/40 p-4">
            <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4 text-xs">
                ...
                <p>Mobile<br /><b>{mobile}</b></p>
                <p>Aadhaar<br /><b>•••• {aadhaar}</b></p>
                <p>Bank<br /><b>{initialBanks.find((b) => b.id === bankId)?.name || "—"}</b></p>
                <p>Portal<br /><b>{initialPortals.find((p) => p.id === portalId)?.name || "—"}</b></p>
              ...
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button type="button" onClick={recordTransaction} disabled={!valid || busy} className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-black text-white disabled:opacity-50">{busy ? "Processing…" : "Approve & Record"}</button>
              </div>
            </div>
          </div>
        )}

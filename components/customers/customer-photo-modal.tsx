...
export default function CustomerPhotoModal({
  ...
  async function uploadPhoto(file: File) {
    ...
    try {
      ...
      const path = `customer-${customerId}-${Date.now()}.${ext}`;
  ...
  async function save() {
    ...
    onSaved(preview);
    logAudit({ action: "upload", entity: "customer", entity_id: customerId, description: "Customer photo updated" });
  ...
  async function remove() {
    ...
    onSaved(null);
    logAudit({ action: "upload", entity: "customer", entity_id: customerId, description: "Customer photo removed" });
      ...
      footer={
        <div className="flex justify-end gap-2">
          ...
          <button
            type="button"
            onClick={save}
            disabled={saving || uploading || preview === photoUrl}
    ...
    >
      <div className="relative bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#020617] px-6 pb-6 pt-8 text-center">
        ...
        <div className="relative mx-auto h-28 w-28">
          ...
          <h2 className="mt-4 text-lg font-bold text-white">{name || "Customer"}</h2>
          <p className="mt-0.5 text-xs text-[#94a3b8]">Customer photo</p>
        </div>

...
export default function IncomeReportClient({ rows, from, to }: { rows: Row[]; from: string; to: string }) {
  ...
  const serviceFees = filteredRows.reduce((s, r) => s + r.fee + r.portalCharge, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900">
        ...
        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Service Type<select value={serviceFilter} onChange={(e) => setServiceFilter(e.target.value)} className="mt-1 block min-w-48 rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-950"><option value="all">All Services</option>{serviceOptions.map((service) => <option key={service} value={service}>{label(service)}</option>)}</select></label>
      </form>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-white/10 dark:bg-slate-900">
        ...
        <select defaultValue="" onChange={(e) => { markSelected(e.target.value as Mark); e.currentTarget.value = ""; }} disabled={selectedVisibleRows.length === 0} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold disabled:opacity-50 dark:border-white/10 dark:bg-slate-950"><option value="">Mark selected…</option><option value="reviewed">Reviewed</option><option value="verified">Verified</option><option value="follow-up">Follow-up</option><option value="reconciled">Reconciled</option><option value="attention">Needs Attention</option></select>

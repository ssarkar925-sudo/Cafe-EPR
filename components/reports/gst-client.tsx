...
export default function GstReportClient({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-5 dark:border-white/10">
        <div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 text-xs font-black text-emerald-700 dark:text-emerald-300">
        ...
        {/* GSTIN Profile Badge */}
        <div className="card-glow-indigo flex items-center gap-3 rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-sm dark:border-white/10 dark:bg-slate-900">
          <div className="icon-box-3d flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400 shadow-xs">
            <Building2 className="h-5 w-5" />
          </div>
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Supplier GSTIN</p>
            <p className="font-mono text-xs font-black tracking-wider text-slate-900 dark:text-white">
      ...
      <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/20 dark:text-amber-200 flex items-start gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs dark:border-white/10 dark:bg-slate-900">
        <div className="flex flex-wrap items-center gap-1.5">
          <button
            onClick={() => handlePeriodChange("fy-current")}
            className={`rounded-xl px-3 py-1.5 text-xs font-bold transition duration-150 active:scale-95 ${
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500 font-bold dark:text-slate-400">From:</span>
          <input
            type="date"
            value={customStart}
            onChange={(e) => setCustomStart(e.target.value)}
            className="rounded-xl border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-800 dark:border-white/10 dark:bg-slate-950 dark:text-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none"
          />
          <span className="text-xs text-slate-500 font-bold dark:text-slate-400">To:</span>
          <input
            type="date"
            value={customEnd}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="card-glow-indigo relative overflow-hidden rounded-2xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/[0.06] via-white to-white p-5 shadow-xs transition hover:scale-[1.02] duration-150 dark:border-indigo-500/30 dark:from-indigo-950/25 dark:via-slate-900 dark:to-slate-900">
          <p className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Total Taxable Turnover</p>
          <p className="mt-2 text-2xl font-black font-mono tracking-tight tabular-nums text-slate-950 dark:text-white">{inr(summary.total_taxable_value)}</p>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Gross taxable outward base</p>
        </div>

        <div className="card-glow-indigo relative overflow-hidden rounded-2xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/[0.06] via-white to-white p-5 shadow-xs transition hover:scale-[1.02] duration-150 dark:border-indigo-500/30 dark:from-indigo-950/25 dark:via-slate-900 dark:to-slate-900">
          <p className="text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400">Central Tax (CGST)</p>
          <p className="mt-2 text-2xl font-black font-mono tracking-tight tabular-nums text-indigo-900 dark:text-indigo-200">{inr(summary.total_cgst)}</p>
          <p className="mt-1 text-xs text-indigo-700/80 dark:text-indigo-400">Intra-state Central component</p>
        </div>

        <div className="card-glow-purple relative overflow-hidden rounded-2xl border border-purple-500/20 bg-gradient-to-br from-purple-500/[0.06] via-white to-white p-5 shadow-xs transition hover:scale-[1.02] duration-150 dark:border-purple-500/30 dark:from-purple-950/25 dark:via-slate-900 dark:to-slate-900">
          <p className="text-xs font-black uppercase tracking-wider text-purple-600 dark:text-purple-400">State Tax (SGST)</p>
          <p className="mt-2 text-2xl font-black font-mono tracking-tight tabular-nums text-purple-900 dark:text-purple-200">{inr(summary.total_sgst)}</p>
          <p className="mt-1 text-xs text-purple-700/80 dark:text-purple-400">Intra-state State component</p>
        </div>

        <div className="card-glow-emerald relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/[0.06] via-white to-white p-5 shadow-xs transition hover:scale-[1.02] duration-150 dark:border-emerald-500/30 dark:from-emerald-950/25 dark:via-slate-900 dark:to-slate-900">
          <p className="text-xs font-black uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Total Output Tax Liability</p>
          <p className="mt-2 text-2xl font-black font-mono tracking-tight tabular-nums text-emerald-700 dark:text-emerald-300">{inr(summary.total_output_tax)}</p>
          <p className="mt-1 text-xs font-bold text-emerald-700/90 dark:text-emerald-400">CGST + SGST + IGST net liability</p>
        </div>
      </div>
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 whitespace-nowrap border-b-2 py-3 px-3 text-xs font-black transition-all duration-150 active:scale-95 ${
                  isActive
      ...
      {activeTab === "export" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="card-glow-indigo rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-4 dark:border-white/10 dark:bg-slate-900">
            <h4 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-2">
              <span className="icon-box-3d flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400">
            ...
            <div className="space-y-2 pt-1">
              <button
                ...
                className="w-full flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50/80 px-3.5 py-2.5 text-xs font-bold text-slate-700 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 transition duration-150 active:scale-95 dark:border-white/10 dark:bg-slate-800/80 dark:text-slate-300 dark:hover:bg-indigo-950/40 dark:hover:text-indigo-300"
              >
                <span>Table 4A: B2B Invoices CSV</span>
                <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400">Download 📥</span>
              </button>
              <button
                onClick={() =>
                  downloadCsv(
          ...
          <div className="card-glow-emerald rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-4 dark:border-white/10 dark:bg-slate-900">
            <h4 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-2">
              <span className="icon-box-3d flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400">
            ...
            <div className="pt-2">
              <Link
                ...
                className="btn-3d-tactile-primary inline-flex items-center gap-2 px-4 py-2.5 text-xs font-black shadow-md"
              >

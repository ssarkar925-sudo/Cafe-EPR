...
function applyDesignVariables(design: DesignStyle, isDark: boolean) {
  ...
  const page = isDark ? (design === "midnight" || design === "neon" ? preset.page : "#0b1120") : preset.page;
  const surface = isDark ? (design === "neon" ? "#0d1520" : "#111827") : preset.surface;
  ...
  root.style.setProperty("--surface-soft", isDark ? "#0f172a" : page);
  root.style.setProperty("--ink", isDark ? "#f8fafc" : "#172033");
  root.style.setProperty("--muted", isDark ? "#94a3b8" : "#667085");
  root.style.setProperty("--line", isDark ? "rgba(255,255,255,.10)" : "rgba(15,23,42,.085)");
...
export default function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onChange = () => {
      if (getStoredTheme() === "system") {
        ...
        applyTheme("system", getStoredAccent(), getStoredDensity(), getStoredFontScale(), getStoredDesign());
      }
    };
  ...
  function handleSetFontScale(next: FontScale) {
    ...
    try { localStorage.setItem(FONT_SCALE_KEY, next); } catch {}
    applyTheme(theme, accent, density, next, design);
  }

  function handleSetDesign(next: DesignStyle) {
    setDesignState(next);
    try { localStorage.setItem(DESIGN_KEY, next); } catch {}
